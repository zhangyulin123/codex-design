...
<!-- Hero -->
<div align="center">
  <b style="font-size: 1.15em;">The Codex design language (a terminal-native dark theme) + a service-oriented sidebar workbench</b><br /><br />
  <a href="https://www.npmjs.com/package/codex-design"><img alt="npm version" src="https://img.shields.io/npm/v/codex-design" /></a>
  <a href="https://www.npmjs.com/package/codex-design"><img alt="npm downloads" src="https://img.shields.io/npm/dm/codex-design" /></a>
  ...
  <a href="https://www.npmjs.com/package/@deepseek-ai/dsh?activeTab=versions"><img alt="Supported DSH versions (codex-design v0.19.0): 0.1.2-rc.1+" src="https://img.shields.io/badge/DSH-0.1.2--rc.1%2B-4d6bfe" /></a>
...
> 📦 **This package is the merged successor**: `codex-design` **`0.19.0`** folds **the Codex design language (a terminal-native dark theme)** and the **complete VSCode-like sidebar workbench of the former `dsh-better-sidebar` v0.18.0** into **ONE package** — every workbench capability plus a whole-UI Codex theme and session management (one-click delete / archived-session manager). Upgrading from the old package: see [MIGRATION.md](./MIGRATION.md).
> 🔗 **Repository TBD**: the badges and links below still point at the pre-merge repository (`github.com/omdsh-dev/DSH-better-sidebar`); `package.json` `repository.url` and the plugin id contain the `your-org` placeholder and will be filled in once the new home is decided — see [MIGRATION.md §Post-merge TODOs](./MIGRATION.md#7-合并后待办post-merge-todos).
...
<div align="center">
  ...
  <img alt="codex-design workbench" src="https://github.com/user-attachments/assets/dfdb875e-a1a8-4d4b-8340-353736b1708f" />
</div>

## 📑 Contents

- [✨ Features](#-features)
- [🎨 Codex Theme](#-codex-theme) · [🧹 Session Management](#-session-management-one-click-delete--archived)
...
The whole-UI theme absorbed in the merge: it switches the Harness interface to a calm, **terminal-native** look.

- **What it is**: an **inspired interpretation** of the **OpenAI Codex design language** (**not** an official token set). It registers a switchable `codex` theme covering the main UI's background / text / buttons / brand color / code highlighting.
- **How to apply / restore**: **Settings → General → "Codex theme"** → click "**Apply Codex theme**" to switch immediately; click "**Restore system light/dark**" to go back.
- **Palette highlights**:
  - Canvas **`#0b0d10`** (near-black), layered surfaces **`#12151a` / `#16191f` / `#1a1f27`**
  - Signature green accent **`#3ecf8e`** (brand / active states)
  - Grayscale text **`#e6e9ee`** (primary) / **`#b7bec8`** (secondary) / **`#8b929d`** (muted)
  - **Hairline borders instead of shadows** (layering by 1px rules, not drop shadows)
  - An overall **mono-first feel** (monospaced-leaning labels and numerals)
- **Persists across restarts**: the enabled flag is persisted through a host route and restored automatically after a Harness restart.
- **It does not fight other themes**: while active it **asks other theme plugins (e.g. catppuccin) to stand down** (switching them to follow-system) so two themes never wrestle over the preference.
- **The sidebar inherits it automatically**: every visual value in the sidebar and its built-in views **consumes only `--dsw-alias-*` / `--dsw-font-*` / `--ds-*` design tokens** with no hard-coded colors — so switching the theme restyles the workbench too, and the two never clash.

> Details: [docs/codex/README.zh-CN.md](./docs/codex/README.zh-CN.md) (feature doc, Chinese).

## 🧹 Session Management: One-Click Delete & "Archived"

Two session-management capabilities absorbed in the merge. Both are built on the Harness's **own** session APIs — no after-the-fact file surgery:

- **One-click delete on session rows**: every session row in the sidebar gains a **trash** button that permanently deletes that session. It calls the Harness's own session-delete capability (the same API as the row's "More → Delete session"), and the host performs the full teardown (releasing the agent, deleting persistence, deregistering from the workspace and the archived set).
- **The "Archived" manager**: the **"Archived"** entry at the **bottom of the sidebar (next to the Settings button)** lists every archived session with **restore (unarchive)** and **delete**. The core UI has no view/unarchive affordance once a session is archived; this plugin's "restore" reads and writes the workspace registry's archived set through a host route.
...
> 📌 **This package is `0.19.0` (the merged release)**: it targets DSH **0.1.2-rc.1+** (npm dist-tag `latest`) and drops 0.1.0-rc.8 ~ 0.1.1-rc.2 — stable-DSH (≤ 0.1.1-rc.2) users should stay pinned to `codex-design@0.18.0` (the pre-merge sidebar code line; `@latest` now points at `0.19.0`).

```sh
dsh plugin --profile web add codex-design@latest           # first run fails: pnpm 11 blocks node-pty build scripts (the dependency is still written)
cd ~/.dsh/profiles/web && pnpm approve-builds --all        # allow the build scripts (node-pty still needs it; re-runs the install automatically)
dsh plugin --profile web add codex-design@latest           # re-run succeeds
```
...
```text
Install the codex-design plugin (a DSH sidebar workbench + Codex dark theme):
1. Run: dsh plugin --profile web add codex-design@latest (the first run fails because pnpm 11 blocks node-pty build scripts — that's expected)
2. In ~/.dsh/profiles/web run: pnpm approve-builds --all (allows the build scripts and re-runs the install)
3. Run the add command again: dsh plugin --profile web add codex-design@latest
4. When done, remind me to hard-refresh the browser (Cmd/Ctrl+Shift+R)
If anything fails, check the troubleshooting table in the README at https://github.com/omdsh-dev/DSH-better-sidebar (the pre-merge historical docs).
```

**Option 3: one-shot script** — from a clone of this repo, run `bash scripts/install.sh` (macOS / Linux / Windows Git Bash; native Windows uses `install.ps1`; `-h` for options) — it automates add → approve-builds → re-run, and **idempotently removes the old `better-sidebar` manual mount line** (avoiding a double-mount).

<details>
<summary><b>Updating</b></summary>

```sh
dsh plugin --profile web add codex-design@latest
```

or bump the version in `~/.dsh/profiles/web/package.json` and run `pnpm install`. Then hard-refresh the browser (Cmd/Ctrl+Shift+R) — client changes do not need a DSH restart.

> Upgrading from `dsh-better-sidebar`: first delete any leftover manual mount line (`- insert: ... better-sidebar ...`) in your profile, then install as above. The namespace and localStorage contracts are deliberately retained, so **your layout / tabs / preferences survive** — see [MIGRATION.md](./MIGRATION.md).
...
| Two sidebars on the page | Double-mount. Old hand-written line: `~/.dsh/profiles/web/cordis.patch.yml` still has `- insert: ... better-sidebar ...` (or an old `codex-design` row) — delete it (a same-id duplicate mount makes the loader fail loudly with `duplicate loader entry id`); `scripts/install.sh` / `install.ps1` clean up both kinds of stale row idempotently. When an aggregate bundle (e.g. `@linxin666/dsh-web-ui-all`) mounts this package under a **different** id, the plugin's own bundle patch backs off automatically (it detects an already-enabled mount of the same package name and does not mount itself) — no manual fix needed; if it still double-mounts, make sure the aggregate bundle precedes `codex-design` in `dsh.profile.bundles`. |
...
| `dsh: command not found` | Install DSH first, or run `npx -y --package @deepseek-ai/dsh dsh plugin --profile web add codex-design@latest`. |
...
```text
1. git clone https://github.com/omdsh-dev/DSH-better-sidebar.git ~/Code/codex-design
   cd ~/Code/codex-design && pnpm install && pnpm build
2. In ~/.dsh/profiles/web/package.json dependencies write "codex-design": "link:<absolute path of the clone>"
...
Update: `git pull && pnpm install && pnpm build` → just hard-refresh the browser (client changes hot-reload; only host-half changes need a DSH restart). To switch back to the npm channel, restore `"codex-design": "^0.19.0"` and re-run `pnpm install`.

> Repository TBD (the `your-org` placeholder): cloning the pre-merge repository is fine for now — replace the URL once the official home is decided.
...
dsh registry enable your-org/codex-design   # registry id = dsh.plugin.json id (your-org is the placeholder)
```
...
```ts
import type {} from 'codex-design'  // triggers the ctx.betterSidebar type merge (the service NAME stays betterSidebar on purpose — see MIGRATION.md)
...
**Supported DSH versions**: <a href="https://www.npmjs.com/package/@deepseek-ai/dsh?activeTab=versions"><img alt="Supported DSH versions (codex-design v0.19.0): 0.1.2-rc.1+" src="https://img.shields.io/badge/DSH-0.1.2--rc.1%2B-4d6bfe" /></a> · full release history on the [Releases](https://github.com/omdsh-dev/DSH-better-sidebar/releases) page (the pre-merge repository)

> 📚 **About the old names below**: the v0.18.0 and older entries are the **historical release records of the pre-merge `dsh-better-sidebar`**, so their package names, version numbers and `github.com/omdsh-dev/DSH-better-sidebar` PR/issue links are **kept verbatim on purpose** (real historical references); the product they describe is now called **codex-design**. The merge itself is documented in [MIGRATION.md](./MIGRATION.md).

### v0.19.0 (the merged release)

> 📦 **One package, two lineages**: this release absorbs the **complete workbench source tree of the former `dsh-better-sidebar` v0.18.0** into **`codex-design`**, and brings the Codex theme and session management in as first-class capabilities. See [MIGRATION.md](./MIGRATION.md) for how the package name / mount id / service and route contracts were handled.

- 🎨 **Codex theme built in**: a whole-UI `codex` theme (near-black canvas + signature green `#3ecf8e` + grayscale text + hairline borders), applicable / restorable from Settings → General, persisted across restarts; while active it asks other theme plugins to stand down; the sidebar follows along because it consumes `--dsw-alias-*` tokens only
- 🧹 **Session management built in**: one-click session delete (trash button on session rows) and the "Archived" session manager (restore / delete), both on the Harness's own session APIs
- 🗂️ **Full workbench capability inherited**: file workbench / previews / embedded browser / real terminal / unified "Changes" / tasks page / Side Chat / dual workbench + free windows / pinned terminals / declarative settings / lazy chunks / 19 third-language overlays
- 🔧 **Identity renamed**: package name, cordis mount id (`codex-design`), plugin id (`your-org/codex-design`), the host plugin `name` export, the invariant companion name, the client bundle id, the CI git user, the aggregate fixture, and the DOM host marker (`data-codex-design`)
- 🤝 **Compatibility constants deliberately retained**: the `ctx.betterSidebar` service name, the `'betterSidebar'` locale namespace, the `/sidebar/*` route prefix, `SIDEBAR_PREFS_NS = 'dsh-better-sidebar'`, all tab / viewer ids, and the legacy DOM marker `data-dsh-better-sidebar` (28+ ecosystem plugins and existing layouts are unaffected)
- 🧱 **The old hand-written implementation retired**: the pre-merge no-build `lib/index.js` + `lib/client.js` are gone (they live in git history) — the capabilities are now TypeScript modules inside `src/`: the **client half in `src/client/codex/`** (`theme.ts` theme engine / `settings.tsx` settings row / `sessions.ts` session & archive data layer / `index.ts` registration glue) and the **host persistence routes in `src/codex-routes.ts`**; `lib/` is now purely the `pnpm build` output directory

### v0.18.0 (pre-merge: the former dsh-better-sidebar stable release)

> 📌 **Stable release** (npm `latest`): this release targets **DSH 0.1.2-rc.1+ only** (peer floor `^0.1.2-rc.1`); 0.1.0-rc.8 ~ 0.1.1-rc.2 are not supported — stable-DSH users should stay pinned to `dsh-better-sidebar@0.17.1`, hosts on 0.1.2-alpha.x keep `dsh-better-sidebar@alpha` (v0.18.0-alpha.0). (This paragraph is the **pre-merge** release note; the original names and version numbers are kept so it still matches the historical publication.)
...
**Architecture**: a single npm package with host/client halves — host (`src/index.ts`): `/sidebar/api/*` JSON API, `/sidebar/file` media route, `/sidebar/html` preview route, `/sidebar/ws/terminal` WebSocket (fs / git / pty / preview, all session-scoped with a trust fence); client (`src/client/index.tsx`): portal sidebar + views + interception; state persisted per session in localStorage. The absorbed Codex theme and session management follow the same split: the **client half lives in `src/client/codex/`** (`theme.ts` theme engine and the `CODEX_TOKENS` palette, `settings.tsx` settings row, `sessions.ts` session & archive data layer, `index.ts` registration glue) and the **host persistence routes in `src/codex-routes.ts`** (wired into the `/sidebar/api` dispatcher in `src/index.ts`), with client-side API wrappers in `src/client/api.ts` and the guard test in `tests/codex-theme.spec.ts`. Organized per DSH official conventions (no default export, dual client bundles); no dependency on npm / checkout at runtime (`@deepseek-ai/*` provided by the web profile). See [MIGRATION.md](./MIGRATION.md) for the pre/post-merge identity and compatibility contracts.
...
- See [`AGENTS.md`](./AGENTS.md) for the repository rules (hard constraints, retained compatibility constants, CI lanes, release flow) and [`MIGRATION.md`](./MIGRATION.md) for the merge record and post-merge TODOs

## ⭐ Star History

> The chart below still points at the pre-merge repository (star-history fetches by repo name); replace the `repos=` argument once `your-org/codex-design` is settled.
