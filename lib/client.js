window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // --- Delete-session shortcut -------------------------------------------
    // Current Harness builds own session deletion (`delete.session` in the row
    // menu, backed by ctx.sessions.delete()). This plugin only adds a one-click
    // TRASH shortcut on each row that calls that SAME native API — no file
    // surgery, so the Host performs the complete teardown. Rows are matched by
    // the action button's aria-label and the id resolved from the sessions store.
    // Unarchive has no native UI, so it keeps this plugin's own host route.
    const UNARCHIVE_ROUTE_PATH = '/codex-design/session/unarchive'
    // Fallback delete route, used only when the native remote is unwired.
    const DELETE_ROUTE_PATH = '/codex-design/session'

    // Delete one session, preferring the Harness's native API. Some builds ship a
    // client whose session-delete remote is not wired (`this.remote.session.delete
    // is not a function`), so fall back to this plugin's own host route, which
    // mirrors the native teardown (persistence delete + registry forgetSession).
    async function deleteSession(sessionsService, id) {
      let nativeError
      try {
        if (!sessionsService || typeof sessionsService.delete !== 'function') {
          throw new Error('native session delete unavailable')
        }
      ...
      if (!payload || payload.ok !== true) {
        throw new Error(payload?.error || nativeError?.message || 'delete failed')
      }
    }
    ...
    function resolveSessionId(sessions, rowTitle) {
      ...
      // The ROW renders the summary's `displayTitle` (derived when a session has
      // no stored title, e.g. a brand-new one), while `title` is present only for
      // sessions that actually carry one. Match on the displayed value first.
      ...
      try {
        ...
        for (const id of ids) {
          if (titleOf(byId[id]) === rowTitle) return String(id)
        }
        for (const id of ids) {
          if (norm(titleOf(byId[id])) === target) return String(id)
        }
        // Also accept the raw stored title, in case the row shows that instead.
        for (const id of ids) {
        ...
        let count = 0
        for (const id of ids) {
          const candidate = norm(titleOf(byId[id]))
    ...
    // Read the EXACT session id off a rendered row. The row's own dragstart
    // handler publishes it as `text/plain` in the drag payload, so dispatching a
    // synthetic dragstart exposes the id directly — no title guessing, and it stays
    // correct even for a brand-new session whose title was not yet derived.
    // Returns null when the row is not draggable (then callers fall back to a
    // title match).
    function sessionIdFromRow(row) {
      if (!row || typeof DataTransfer !== 'function' || typeof DragEvent !== 'function') return null
      try {
        const data = new DataTransfer()
        row.dispatchEvent(new DragEvent('dragstart', { bubbles: true, cancelable: true, dataTransfer: data }))
        const id = data.getData('text/plain')
        row.dispatchEvent(new DragEvent('dragend', { bubbles: true, cancelable: true, dataTransfer: data }))
        return id || null
      } catch {
        return null
      }
      ...
      const observe = () => {
        ...
        document.querySelectorAll('button[aria-label]').forEach((anchor) => {
          ...
          del.addEventListener('click', async (event) => {
            ...
            // Read the title fresh: a row's title changes after injection (a new
            // session starts blank, then takes its title), so a captured value can
            // be stale. The id itself comes off the row, not from any title match.
            const liveTitle = rowTitleFromActionButton(anchor) || rowTitle
            const name = liveTitle || '该会话'
            const row = anchor.closest('[role="treeitem"]')
            const targetId = sessionIdFromRow(row) || resolveSessionId(sessions, liveTitle)
            if (!targetId) {
              await alertDialog(
                deleteText + '：找不到该会话。请改用该行「…」菜单里的「删除会话」，或刷新后重试。',
                deleteText
              )
              return
            }
            ...
            if (!ok) return
            try {
              // Prefer the Harness's OWN session-delete API (the row menu's
              // "删除会话" uses the same call); fall back to the plugin route when
              // this build's native remote is unwired.
              await deleteSession(sessions, targetId)
            } catch (error) {
              await alertDialog(deleteText + '失败：' + (error?.message || String(error)), deleteText)
            }
          })
    ...
    function createArchivedManager(ctx) {
      ...
      async function removeArchived(id, name) {
        ...
        if (!ok) return
        try {
          await deleteSession(getSessions(), id)
          emit()
          if (overlay) renderList()
        } catch (error) {
          await alertDialog('删除失败：' + (error?.message || String(error)), '删除会话')
        }
      }
    ...
    // `sessions` is declared so its service is fully wired for this fiber: the
    // delete path reaches the Harness's own session-delete remote through it.
    const inject = ['slots', 'locale', 'theme', 'sessions']
    function apply(ctx) {
      installStyles()
      ensureOverrideStyle()
      ctx.effect(
      ...
      // Theme guard. The Host owns a DURABLE built-in preference and re-adopts it
      // on its own schedule, which can revert codex to the default long after our
      // scheduled re-applies finish — and on a fresh launch the intent has to be
      // learned from the Host file first. So poll slowly: keep hydrating until the
      // intent is known, then re-apply codex whenever the resolved preference has
      ...
      ctx.effect(() => {
        const timer = setInterval(() => {
          ...
          let current
          try {
            current = theme.getTheme?.()?.preference
          } catch {
            current = undefined
          }
          if (current !== THEME_ID) applyCodex()
        }, 1500)
        return () => clearInterval(timer)
      }, 'codex-design: theme guard')

      // `sessions` is declared in inject, so the service is fully wired here.
      const sessions = ctx.get('sessions')

      // Make the Harness's OWN delete work in this build. Its client calls
      // `ctx.sessions.delete(id)`, which reaches a `session.delete` remote that is
      // not wired here ("this.remote.session.delete is not a function"), so the
      // row menu's "删除会话" fails. Wrap the method: on that failure fall back to
      // this plugin's host route, which runs the SAME Host teardown — including
      // disposing a live agent first, which a bare persistence delete rejects.
      if (sessions && typeof sessions.delete === 'function') {
        const nativeDelete = sessions.delete.bind(sessions)
        const patchedDelete = async (sessionId) => {
          try {
            return await nativeDelete(sessionId)
          } catch (nativeError) {
            const response = await fetch(DELETE_ROUTE_PATH + '?id=' + encodeURIComponent(sessionId), {
              method: 'DELETE',
              credentials: 'same-origin',
              headers: { accept: 'application/json' }
            })
            ...
            // The native path also re-projects its list; a reload is the reliable
            // equivalent for this fallback.
            window.location.reload()
          }
        }
        try {
          sessions.delete = patchedDelete
        } catch {
          /* ignore: a proxy may refuse the assignment */
        }
      }

      // Also add a one-click trash button on each row (same patched API).
