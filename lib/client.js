  ...
  factory: (require) => {
      ...
      const observe = () => {
        ...
        document.querySelectorAll('button[aria-label]').forEach((anchor) => {
          ...
          del.addEventListener('click', async (event) => {
            ...
            if (!ok) return
            try {
              // Use the Harness's OWN session-delete API (the row menu's "删除会话"
              // uses the same call). It performs the complete teardown on the Host:
              // dispose owned agents, drop persistence, and unaccount the session —
              // so no file surgery and no ghost sessions.
              await sessions.delete(targetId)
            } catch (error) {
    ...
    function createArchivedManager(ctx) {
      ...
      async function removeArchived(id, name) {
        ...
        if (!ok) return
        try {
          // Harness-native delete (same call the row menu uses): it disposes owned
          // agents, drops persistence and unaccounts the session from every
          // workspace AND the archive set — so the ghost problem cannot recur.
          await getSessions()?.delete?.(id)
          emit()
          if (overlay) renderList()
        } catch (error) {
