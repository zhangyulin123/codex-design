...
// Fallback session deletion. The preferred path is the Host's OWN
// session-controller command — literally the code the client's `session.delete`
// remote would run — so an owned/live agent is disposed FIRST (a live session
// otherwise refuses deletion), persistence is dropped, and the session is
// unaccounted from every workspace and the archive set. Only when that service is
// absent does this mirror the teardown by hand. Returns { ok } for the browser.
async function handleSessionDelete(ctx, req, res, url) {
  ...
  try {
    ...
    if (controller && typeof controller.delete === 'function') {
      ...
      removeProjectionCache(ctx.get('sessionPersistence'), sessionId)
      sendJson(res, 200, { ok: true, deletedId: sessionId, deleted: true })
      return
    }
    // Mirror the same teardown if the controller service is unavailable: dispose
    // the owned agent first, then drop persistence and unaccount the session.
    const sp = ctx.get('sessionPersistence')
    const agents = ctx.get('agents')
    if (agents && typeof agents.disposeOwned === 'function') await agents.disposeOwned(sessionId)
    const liveInAgents = agents && typeof agents.get === 'function' && agents.get(sessionId) !== undefined
    const sessionStore = ctx.get('sessions')
    const liveInStore = sessionStore && typeof sessionStore.get === 'function' && sessionStore.get(sessionId) !== undefined
    if (liveInAgents || liveInStore) {
      sendJson(res, 200, { ok: false, error: '该会话仍在运行，请先停止它再删除' })
      return
    }
    let deleted = false
    if (sp && typeof sp.delete === 'function') deleted = Boolean(await sp.delete(sessionId))
    const registry = ctx.get('workspaceRegistry')
    ...
    sendJson(res, 200, { ok: true, deletedId: sessionId, deleted })
  } catch (error) {
    sendJson(res, 200, {
      ok: false,
