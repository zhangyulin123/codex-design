#!/usr/bin/env bash
# =============================================================================
# codex-design 一键安装脚本（官方 CLI 方式，macOS / Linux / Windows Git Bash）
#
# 通过 DSH 官方插件命令安装 npm 包并自动挂载：
#   dsh plugin --profile web add codex-design@<version>
#
...
# - 回滚：dsh plugin --profile web remove codex-design，或把 profile 依赖
#   改回 "codex-design": "link:<路径>" 再 pnpm install。
# =============================================================================
set -euo pipefail
...
if (!/^\s*-\s+codex-design\s*$/m.test(t)) {
...
while (i < lines.length) {
  ...
  if (/^[ \t]*- insert:\s*$/.test(line)) {
    ...
    if (block.some((l) => /id:\s*(?:codex-design|better-sidebar)\b/.test(l))) {
...
[ "$MOUNT_RESULT" = "removed" ] \
  && say "已从 $PATCH_YML 移除旧的手动挂载行（codex-design / 合并前的 better-sidebar，bundle 通道接管挂载）" \
  || say "无旧手动挂载行，跳过"
