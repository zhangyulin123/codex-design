...
<!-- Hero -->
<div align="center">
  <b style="font-size: 1.15em;">Codex 设计语言（终端原生暗色主题）＋ 一套服务化的侧边栏工作台</b><br /><br />
  ...
  <a href="https://www.npmjs.com/package/@deepseek-ai/dsh?activeTab=versions"><img alt="支持的 DSH 版本（codex-design v0.19.0）：0.1.2-rc.1+" src="https://img.shields.io/badge/DSH-0.1.2--rc.1%2B-4d6bfe" /></a>
...
> 📦 **本包是合并后的继任者**：`codex-design` **`0.19.0`** 把 **Codex 设计语言（终端原生暗色主题）** 与原 **`dsh-better-sidebar` v0.18.0** 的**完整 VSCode 风格侧边栏工作台**合成**一个包**——工作台能力全覆盖，并新增整站 Codex 主题与会话管理（一键删除 / 已归档管理）。从旧包升级请看 [MIGRATION.md](./MIGRATION.md)。
> 🔗 **仓库地址与 npm 占位符待定稿**：下方徽章与链接仍指向合并前的仓库（`github.com/omdsh-dev/DSH-better-sidebar`）；`package.json` `repository.url` 与插件 id 里的 `your-org` 是占位符，待替换后统一回填，见 [MIGRATION.md §合并后待办](./MIGRATION.md#7-合并后待办post-merge-todos)。
...
<div align="center">
  <img alt="codex-design 工作台截图" src="https://github.com/user-attachments/assets/991c4b70-d45a-461f-a8c8-0a28b4218e60" />
...
- [✨ 功能一览](#-功能一览)
- [🎨 Codex 主题](#-codex-主题) · [🧹 会话管理](#-会话管理一键删除与已归档)
- [🚀 安装](#-安装)
- [🖼️ 特性巡礼](#-特性巡礼)
- [🌐 插件生态](#-插件生态)
- [🆕 最近更新](#-最近更新)
...
合并进来的整站主题：把 Harness 界面切换成**终端原生**的冷静观感。

- **它是什么**：对 **OpenAI Codex 设计语言**的**启发式解读**（**不是**官方令牌集）。注册一个可切换的 `codex` 主题，覆盖主界面的背景 / 文字 / 按钮 / 品牌色 / 代码高亮等。
- **怎么应用 / 还原**：**设置 → 通用 → 「Codex 主题」** → 点「**应用 Codex 主题**」立即生效；点「**恢复系统 亮/暗 跟随**」一键还原。
- **调色板要点**：
  - 画布 **`#0b0d10`**（近黑），分层表面 **`#12151a` / `#16191f` / `#1a1f27`**
  - 标志性绿点缀 **`#3ecf8e`**（品牌 / 激活态）
  - 灰阶文本 **`#e6e9ee`**（主）/ **`#b7bec8`**（次）/ **`#8b929d`**（弱）
  - **发丝边框代替阴影**（hairline borders，不用大投影分层）
  - **等宽优先**的整体气质（mono-first 的标签与数字观感）
- **跨重启持久化**：是否启用由宿主路由持久化，重启 Harness 后自动恢复。
- **不抢主题**：激活时会**主动请其它主题插件（如 catppuccin）让位**（切到跟随系统），避免两个主题互抢。
- **侧边栏自动继承**：侧边栏与所有内置视图的视觉值**只消费 `--dsw-alias-*` / `--dsw-font-*` / `--ds-*` 设计令牌**，不硬编码颜色——所以主题一换，工作台跟着换，两者不会打架。

> 详细说明见 [docs/codex/README.zh-CN.md](./docs/codex/README.zh-CN.md)（功能文档）。

## 🧹 会话管理：一键删除与「已归档」

合并进来的两项会话管理能力，都建立在 Harness **自带**的会话 API 上，不做事后文件手术：

- **会话行一键删除**：侧边栏每个会话行多出一个**垃圾桶**按钮，点它即永久删除该会话。走的是 Harness 自带的会话删除能力（与行内「更多 → 删除会话」同一个 API），由宿主完成完整拆除（释放 agent、删除持久化、从工作区与归档集合注销）。
- **「已归档」管理器**：侧边栏**底部（设置按钮旁）**的「已归档」入口列出所有已归档会话，支持**恢复（取消归档）**与**删除**。核心 UI 归档后没有查看/取消归档入口，本插件的「恢复」通过宿主路由读写 workspace registry 的归档集合补上。
...
> 📌 **本包版本 `0.19.0`（合并版）**：适配 DSH **0.1.2-rc.1+**（npm dist-tag `latest`），不再支持 0.1.0-rc.8 ~ 0.1.1-rc.2——DSH stable（≤ 0.1.1-rc.2）用户请固定安装 `codex-design@0.18.0`（合并前的 sidebar 代码线；`@latest` 已由 `0.19.0` 接管）。**前提是 npm 上已发布该包**：`codex-design` 的首次发布依赖 [MIGRATION.md §合并后待办](./MIGRATION.md#7-合并后待办post-merge-todos) 里的 npm Trusted Publishing 配置；发布前请用下面的**方式三（源码）或 plugin-registry 通道**。
...
```text
帮我安装 codex-design 插件（DSH 侧边栏工作台 + Codex 暗色主题），步骤：
1. 执行 dsh plugin --profile web add codex-design@latest（首次会被 pnpm 11 拦截 node-pty 构建脚本而失败，属正常）
2. 在 ~/.dsh/profiles/web 下执行 pnpm approve-builds --all（放行构建脚本，会自动重跑安装）
3. 再次执行 dsh plugin --profile web add codex-design@latest
4. 完成后提醒我硬刷新浏览器（Cmd/Ctrl+Shift+R）
遇到报错先查 https://github.com/omdsh-dev/DSH-better-sidebar README 的常见问题表（合并前的历史文档）。
```

**方式三：一键脚本**——克隆本仓库后执行 `bash scripts/install.sh`（macOS / Linux / Windows Git Bash；Windows 原生环境用 `install.ps1`；`-h` 查看参数），自动完成 add → 放行构建脚本 → 重跑安装，并**幂等清理旧的 `better-sidebar` 手动挂载行**（避免双挂载）。
...
> 从 `dsh-better-sidebar` 升级过来：先删掉 profile 里残留的旧手动挂载行（`- insert: ... better-sidebar ...`），再按上面安装。命名空间与 localStorage 契约刻意保留，所以**布局 / Tab / 偏好不会丢**——详见 [MIGRATION.md](./MIGRATION.md)。
...
| 页面出现**两个侧边栏** | 双挂载。旧的手动挂载行：`~/.dsh/profiles/web/cordis.patch.yml` 还留着 `- insert: ... better-sidebar ...`（或旧的 `codex-design` 行），删掉那段（同 id 重复挂载 loader 会直接报 `duplicate loader entry id`）；`scripts/install.sh` / `install.ps1` 会幂等清理这两种旧行。聚合包（如 `@linxin666/dsh-web-ui-all`）以**不同 id** 挂载本包时，插件自身 bundle patch 会自动退让（检测到已有启用中的同包名挂载就不挂自己），无需手动处理；若仍双挂载，先确认聚合包的 bundle 顺序在 `codex-design` 之前。 |
...
| 提示 `dsh: command not found` | 先安装 DSH；或直接用 `npx -y --package @deepseek-ai/dsh dsh plugin --profile web add codex-design@latest`。 |
...
2. ~/.dsh/profiles/web/package.json 的 dependencies 写 "codex-design": "link:<克隆目录绝对路径>"
...
更新：`git pull && pnpm install && pnpm build` → 硬刷新浏览器即可（client 改动热加载生效，无需重启 DSH；host 半改动才需重启）。切回 npm 通道时，把依赖改回 `"codex-design": "^0.19.0"` 再 `pnpm install`。

> 仓库地址待定稿（`your-org` 占位）：克隆 URL 沿用合并前仓库即可，官方仓库落定后请替换。
...
dsh registry enable your-org/codex-design   # registry id = dsh.plugin.json 的 id（your-org 为待替换占位）
```
...
```ts
import type {} from 'codex-design'  // 触发 ctx.betterSidebar 类型合并（服务名刻意保持 betterSidebar，见 MIGRATION.md 兼容性约定）
...
**支持的 DSH 版本**：<a href="https://www.npmjs.com/package/@deepseek-ai/dsh?activeTab=versions"><img alt="支持的 DSH 版本（codex-design v0.19.0）：0.1.2-rc.1+" src="https://img.shields.io/badge/DSH-0.1.2--rc.1%2B-4d6bfe" /></a> · 完整发布历史见 [Releases](https://github.com/omdsh-dev/DSH-better-sidebar/releases)（合并前仓库）

> 📚 **关于下文的旧名字**：v0.18.0 及更早的条目是**合并前 `dsh-better-sidebar` 的历史发布记录**，其中的包名、版本号与 `github.com/omdsh-dev/DSH-better-sidebar` 的 PR / issue 链接**故意保留原样**（真实历史引用）；正文描述的"产品"现在就叫 **codex-design**。合并本身见 [MIGRATION.md](./MIGRATION.md)。

### v0.19.0（合并版）

> 📦 **一个包，两段血脉**：本版把原 **`dsh-better-sidebar` v0.18.0** 的完整工作台源码树并入 **`codex-design`**，并把 Codex 主题与会话管理作为一等能力收进来。包名 / 挂载 id / 服务与路由契约的处理见 [MIGRATION.md](./MIGRATION.md)。

- 🎨 **Codex 主题内置**：整站 `codex` 主题（近黑画布 + 标志性绿 `#3ecf8e` + 灰阶文本 + 发丝边框），设置 → 通用可应用 / 还原，跨重启持久化；激活时会请其它主题插件让位；侧边栏因只消费 `--dsw-alias-*` 令牌而自动跟随
- 🧹 **会话管理内置**：会话行一键删除（垃圾桶按钮）与「已归档」会话管理器（恢复 / 删除），均走 Harness 自带的会话 API
- 🗂️ **工作台能力全量继承**：文件工作台 / 预览 / 内嵌浏览器 / 真实终端 / 统一「文件变动」/ 任务页 / 侧边对话 / 双工作台 + 自由窗口 / 固定终端 / 声明式设置 / 懒加载 chunk / 19 语言第三方覆盖
- 🔧 **身份改名**：包名、cordis 挂载 id（`codex-design`）、插件 id（`your-org/codex-design`）、宿主插件 `name` 导出、不变量伴生名、client bundle id、CI git user、聚合 fixture、宿主 DOM 标记（`data-codex-design`）
- 🤝 **兼容常量刻意保留**：`ctx.betterSidebar` 服务名、`'betterSidebar'` 词典命名空间、`/sidebar/*` 路由前缀、`SIDEBAR_PREFS_NS = 'dsh-better-sidebar'`、所有 tab / viewer id、DOM 上的旧标记 `data-dsh-better-sidebar`（28+ 生态插件与已存布局不受影响）
- 🧱 **旧的手写实现退场**：合并前那套无构建的 `lib/index.js` + `lib/client.js` 已删除（存于 git 历史），能力已改写为 TypeScript 模块并入 `src/`——client 半在 `src/client/codex/`（`theme.ts` 主题引擎 / `settings.tsx` 设置行 / `sessions.ts` 会话与归档数据层 / `index.ts` 注册胶水），宿主持久化路由在 `src/codex-routes.ts`；`lib/` 现在只是 `pnpm build` 的产物目录

### v0.18.0（合并前：原 dsh-better-sidebar 的正式版）

> 📌 **正式版**（npm `latest`）：本版仅支持 **DSH 0.1.2-rc.1+**（peer 下限 `^0.1.2-rc.1`）；不再支持 0.1.0-rc.8 ~ 0.1.1-rc.2——DSH stable 用户请固定安装 `dsh-better-sidebar@0.17.1`，停留在 0.1.2-alpha.x 的宿主继续用 `dsh-better-sidebar@alpha`（v0.18.0-alpha.0）。（本段为**合并前**的版本说明，保留原名与版本号以对应历史发布。）
...
**架构**：单 npm 包、host/client 双半结构——host（`src/index.ts`）：`/sidebar/api/*` JSON API、`/sidebar/file` 媒体路由、`/sidebar/html` 预览路由、`/sidebar/ws/terminal` WebSocket（fs / git / pty / 预览，全部会话级 + 信任围栏）；client（`src/client/index.tsx`）：portal 侧边栏 + 各视图 + 拦截；状态按会话持久化 localStorage。合并进来的 Codex 主题与会话管理同样按这一结构落位：**client 半在 `src/client/codex/`**（`theme.ts` 主题引擎与 `CODEX_TOKENS` 调色板、`settings.tsx` 设置行、`sessions.ts` 会话与归档数据层、`index.ts` 注册胶水），**宿主持久化路由在 `src/codex-routes.ts`**（由 `src/index.ts` 的 `/sidebar/api` 分发器接入），client 侧 API 包装在 `src/client/api.ts`，测试守护在 `tests/codex-theme.spec.ts`。插件按 DSH 官方规范组织（无 default 导出、双 client bundle），运行期不依赖 npm / checkout（`@deepseek-ai/*` 由 web profile 提供）。合并前后的身份与兼容约定见 [MIGRATION.md](./MIGRATION.md)。
...
- 仓库工作规范见 [`AGENTS.md`](./AGENTS.md)（含仓库硬约束、保留的兼容常量与 CI 说明）；合并前后的身份映射与待办见 [`MIGRATION.md`](./MIGRATION.md)

## ⭐ Star History

> 下图仍指向合并前的仓库（star-history 按仓库名取数）；待 `your-org/codex-design` 落定后替换 `repos=` 参数。
