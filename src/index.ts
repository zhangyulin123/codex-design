/**
 * codex-design host half: the /sidebar JSON API (explorer listing, file
...
function buildApi(
  ...
  return {
    ...
    // Codex feature set: durable theme intent + the session-management
    // fallbacks (unarchive, and the delete path for builds whose native
    // session.delete remote is unwired). Same fence, same envelope.
    ...buildCodexApi(ctx),
  }
}
...
export function apply(ctx: Context, config?: SidebarConfig): void {
  ...
  if (nodePty === null) {
    ...
    ctx.logger?.warn(`[codex-design] node-pty (${DSH_NODE_PTY_RANGE}) failed to load: ${detail}`)
  }
  ...
  ctx.inject(['settings'], (sctx) => {
    ...
    // (SettingsNamespaceInput); the 'codex-design' literal passes, so the
  ...
  }), 'codex-design: /sidebar/api routes')
  ...
  }), 'codex-design: /sidebar/upload route')
  ...
  ctx.effect(() => registerBundleRoute(ctx, fence), 'codex-design: /sidebar/bundle chunk route')
  ...
  }), 'codex-design: /sidebar/file media route')
  ...
  }), 'codex-design: /sidebar/html preview route')
  ...
  }), 'codex-design: terminal WebSocket')
  ...
  }), 'codex-design: agent-terminals push WebSocket')
  ...
  }), 'codex-design: agent-opens push WebSocket')
