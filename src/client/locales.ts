 ...
 * while the override-aware `t()` here covers codex-design's own
...
export const zh = {
  ...
  openPlugin: '跳转',
  copyInstall: '复制安装命令',
  pluginMdExportDesc: '在 codex-design 的 Markdown 工具栏上新增「导出」按钮：一键把当前 .md 渲染为独立 HTML（表格/代码块/Mermaid 图表内联，布局跟随预览主题）直接保存到同目录，或通过打印对话框导出为 PDF',
  pluginOfficeDesc: '为 codex-design 编辑器提供 Office 三件套预览（.docx / .xlsx / .pptx），把重型 Office 渲染库拆出主包、按需安装',
  pluginFlowglassDesc: '实时会话流程图：三列泳道展示用户、助手与工具调用，支持并行分组、子代理支线、逐层钻取和实时状态；安装 codex-design 后注册原生「流镜」Tab，未安装时保留独立抽屉',
  pluginGitForgeDesc: 'codex-design「Git 凭据」Tab：GitHub/Gitea 等 Forge 账号库 + 按项目授权 + push 策略硬拦；token 仅存本地 secrets，不进模型上下文；提供只读 GitForge 工具与 agent HTTPS credential helper',
  pluginGithubWorkbenchDesc: 'codex-design「GitHub 工作台」Tab：远端仓库目录树 + Issues / Pull requests / Actions 页签，读之外支持新建 Issue/PR、评论、编辑、关闭重开、squash·merge·rebase 合并（强确认）与重跑/取消 CI；仓库弹层自动拉取有权限列表并支持公开仓搜索；未装 codex-design 时自动降级为独立右侧面板',
  pluginSuhuangScrollDesc: '把本地苏黄共阅 Runtime 接入 DSH 设置与 codex-design，支持模型配置、连接测试和连续阅卷控制；使用前需安装 Suhuang Scroll Runtime 与 codex-design',
  pluginBetterOverleafDesc: 'codex-design 的 Overleaf 标签页：直连 CDP 浏览器登录（支持第三方 Chromium），项目列表/切换，<workspace>/overleaf/ 本地 git 镜像，git 双向同步（API 只读兜底），文件预览走侧边栏工作台',
  pluginGitRemotesDesc: 'codex-design Git 远程 Tab：看分支/上游/ahead-behind，fetch（可 prune）、ff-only pull、确认后才 push。不替换内置 Git 的暂存/提交，也不提供 force-push 或模型自动推送',
  pluginSentinelDesc: '条件驱动的 agent 唤醒系统：文件/进程/端口/HTTP/命令/webhook 传感器，条件达成自动唤醒休眠会话；注册「哨兵」Tab 展示服务器全局监控表',
  pluginServerDeckDesc: '服务器卡片仪表盘：每台服务器一张卡片，展示在线状态、OS、运行时长、CPU/内存/磁盘用量与延迟；点卡片进入 xterm.js 交互终端，支持 ~/.ssh/config 一键导入（自动跳过 Git 托管别名）；安装 codex-design 后注册原生「服务器」Tab，未安装时保留独立抽屉',
  pluginSidebarQaDesc: '基于 codex-design 的划选提问tab分页: 对话划选 → 右侧面板提问 → 同工作区独立追问会话（❓追问·主题）：快速无思考模型压缩主对话上下文后与引文一起注入，不打断主对话；追问可嵌套、可继续、可归档',
  ...
  pluginSshTunnelDesc: 'codex-design「SSH 隧道」Tab：多机主机清单 + 按项目授权 + 密钥本地保管；模型工具 SSHManager（exec/SFTP/会话策略）；中央交互终端与双栏 SFTP',
  ...
  pluginVideoPreviewDesc: '在 codex-design 编辑器内联预览视频文件（.mp4/.webm/.mov/.mkv/.avi 等），自带支持 HTTP Range（206）的 /video 宿主路由，可拖动进度条、不受 20MB mediaLimit 限制',
  pluginCodeNavDesc: '代码预览导航：按文件类型自动识别语言并高亮语法，符号大纲（类/方法/变量筛选 + 一键跳转），文件内查找（全部匹配高亮、上/下一处、区分大小写），接管 codex-design 的代码文件预览',
  pluginDocsPanelDesc: 'DSH 侧边栏里的「全局文档」：全局 Markdown 笔记，任何工作区随时可读——列表点选阅读、悬浮大纲跳转、Chrome / VS Code 外部打开、代码复制，目录可配置（默认 ~/.dsh/docs）',
  pluginEgoBrowserDesc: '把 CitroLabs/ego-lite 接进 DeepSeek Harness 的 agent 浏览器：32 个 ego_* 工具驱动真实 Chromium，侧边栏原生「ego 浏览器」Tab 实时观察 agent 逛的每个页面，可直接点击/拖拽/输入接管；装 codex-design 时自动注册 Tab，没装则退回浮动浮窗',
  pluginBilingualReaderDesc: '在 DSH 侧边栏读论文 PDF：原生 PDF 显示，选中一段文字即用大模型划词翻译，结合上下文、完全隔离主对话，只作阅读辅助',
  codexPageTitle: 'Codex',
  codexThemeTitle: 'Codex 主题',
  codexThemeDesc: '把终端原生的 Codex 配色（近黑画布 + 标志性绿色点缀）应用到整个 Harness 界面。',
  codexThemeApply: '应用 Codex 主题',
  codexThemeRestore: '恢复系统 亮/暗 跟随',
  codexThemeActive: 'Codex 主题已生效',
  codexSessions: '会话',
  codexActiveSessions: '当前会话',
  codexArchived: '已归档',
  codexArchivedEmpty: '暂无已归档会话',
  codexRestore: '恢复',
  codexDelete: '删除',
  codexDeleteConfirm: '确定要永久删除会话「{name}」吗？该会话的对话记录会被删除，且无法恢复。',
  codexUnarchiveFailed: '恢复失败',
  codexDeleteFailed: '删除失败',
  codexTokens: '设计令牌',
  codexTokensDesc: 'Codex 主题覆盖的界面令牌：近黑画布、hairline 描边、终端绿点缀。',
  codexViewerTitle: 'Codex 设计稿',
  codexViewerEmpty: '这个文件里没有可识别的设计令牌。',
}
...
export const en: Record<keyof typeof zh, string> = {
  ...
  pluginMdExportDesc: 'Adds an "Export" button to the Markdown toolbar in codex-design: one click renders the current .md into standalone HTML (tables / code blocks / Mermaid diagrams inlined, layout follows the preview theme) and saves it next to the .md, or exports to PDF via the print dialog',
  pluginOfficeDesc: 'Office-suite preview (.docx / .xlsx / .pptx) for the codex-design editor, keeping the heavy Office render libraries out of the core bundle',
  pluginFlowglassDesc: 'Live session flowgraph with three lanes for user, assistant, and tool calls, plus parallel groups, sub-agent branches, drill-down, and live status; registers a native Flowglass tab when codex-design is installed and keeps its standalone drawer as a fallback',
  ...
  pluginGithubWorkbenchDesc: 'GitHub Workbench tab: remote repo tree + Issues / Pull requests / Actions with full write support — create Issue/PR, comment, edit, close/reopen, squash·merge·rebase merge (strong confirm), re-run/cancel CI; the repo switcher auto-lists accessible repos and searches public ones; falls back to a standalone right-side panel without codex-design',
  ...
  pluginServerDeckDesc: 'Server card dashboard: one card per host showing online status, OS, uptime and CPU/mem/disk usage with latency; click a card to open an interactive xterm.js terminal; one-click ~/.ssh/config import (git-hosting aliases auto-skipped). Registers a native "Servers" tab when codex-design is installed and keeps its standalone drawer as a fallback',
  ...
  pluginSuhuangScrollDesc: 'Connect the local Suhuang Scroll Runtime to DSH settings and codex-design for model configuration, connection tests, and continuous grading controls; requires Suhuang Scroll Runtime and codex-design',
  ...
  pluginVideoPreviewDesc: 'Inline video preview (.mp4/.webm/.mov/.mkv/.avi etc.) for the codex-design editor, backed by a dedicated /video host route with HTTP Range (206) support — scrubbing works and files are not capped by the 20MB mediaLimit',
  pluginCodeNavDesc: 'Code preview navigator: detects the language by file type and highlights syntax, symbol outline (class / method / variable filters + one-click jump), and in-file search (highlight all matches, prev/next, match case) — takes over code file preview in the codex-design editor',
  ...
  pluginEgoBrowserDesc: 'The agent browser for DeepSeek Harness: 32 ego_* tools drive a real Chromium, with a native sidebar "ego browser" tab giving a live view of every page the agent visits — you can click, drag, and type to take over. Registers the tab automatically when codex-design is present, otherwise falls back to a floating bubble',
  pluginBetterOverleafDesc: 'Overleaf tab for codex-design: direct-CDP browser login (third-party Chromium supported), project list/switch, local git mirrors under <workspace>/overleaf/, two-way git sync with read-only API fallback, and file preview through the sidebar workbench',
  ...
  codexPageTitle: 'Codex',
  codexThemeTitle: 'Codex theme',
  codexThemeDesc: 'Apply the terminal-native Codex palette (near-black canvas, signature green accent) to the whole Harness UI.',
  codexThemeApply: 'Apply Codex theme',
  codexThemeRestore: 'Follow system light / dark',
  codexThemeActive: 'Codex theme is active',
  codexSessions: 'Sessions',
  codexActiveSessions: 'Current sessions',
  codexArchived: 'Archived',
  codexArchivedEmpty: 'No archived sessions',
  codexRestore: 'Restore',
  codexDelete: 'Delete',
  codexDeleteConfirm: 'Permanently delete session "{name}"? Its conversation log is removed and cannot be recovered.',
  codexUnarchiveFailed: 'Restore failed',
  codexDeleteFailed: 'Delete failed',
  codexTokens: 'Design tokens',
  codexTokensDesc: 'UI tokens the Codex theme overrides: near-black canvas, hairline strokes, terminal-green accent.',
  codexViewerTitle: 'Codex design',
  codexViewerEmpty: 'No recognizable design tokens in this file.',
}
