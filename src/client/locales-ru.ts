/**
 * Russian (ru) dictionary for the codex-design plugin.
 *
...
export const ru: Record<string, string> = {
  ...
  pluginOfficeDesc: 'Предпросмотр Office (.docx / .xlsx / .pptx) для редактора codex-design; тяжёлые библиотеки рендеринга Office вынесены из основного пакета и устанавливаются по требованию',
  pluginFlowglassDesc: 'Живой граф потока сессии: три дорожки для пользователя, ассистента и вызовов инструментов, с параллельными группами, ветками субагентов, послойным раскрытием и статусом в реальном времени; регистрирует собственную вкладку «Flowglass» при установленном codex-design и сохраняет отдельный лоток как запас',
  ...
  pluginGithubWorkbenchDesc: 'Вкладка «GitHub Workbench»: дерево удалённого репозитория + Issues / Pull requests / Actions с полной поддержкой записи — создание Issue/PR, комментарии, редактирование, закрытие/повторное открытие, merge squash·merge·rebase (с сильным подтверждением), перезапуск/отмена CI; переключатель репозиториев автоматически показывает доступные репозитории и ищет по публичным; без codex-design используется отдельная правая панель',
  pluginMdExportDesc: 'Добавляет кнопку «Экспорт» на панель инструментов Markdown в codex-design: одним кликом рендерит текущий .md в самостоятельный HTML (таблицы/блоки кода/Mermaid-диаграммы встроены, вёрстка следует теме предпросмотра) и сохраняет рядом с .md либо экспортирует в PDF через диалог печати',
  pluginCodeNavDesc: 'Навигация по предпросмотру кода: определяет язык по типу файла и подсвечивает синтаксис, схема символов (фильтры классов/методов/переменных + переход в один клик) и поиск по файлу (подсветка всех совпадений, назад/вперёд, учёт регистра) — берёт на себя предпросмотр файлов кода в редакторе codex-design',
  pluginSuhuangScrollDesc: 'Подключает локальный Suhuang Scroll Runtime к настройкам DSH и codex-design для настройки модели, проверки соединения и управления непрерывной проверкой; требуются Suhuang Scroll Runtime и codex-design',
  pluginBetterOverleafDesc: 'Вкладка Overleaf для codex-design: вход напрямую через CDP-браузер (поддержка стороннего Chromium), список/переключение проектов, локальные git-зеркала в <workspace>/overleaf/, двунаправленная синхронизация git с откатом к API только для чтения, просмотр файлов через рабочую панель сайдбара',
  ...
  pluginVideoPreviewDesc: 'Встроенный предпросмотр видеофайлов (.mp4/.webm/.mov/.mkv/.avi и др.) в редакторе codex-design; обслуживается выделенным хост-маршрутом /video с поддержкой HTTP Range (206) — перемотка работает, а файлы не ограничены mediaLimit в 20 МБ',
  ...
  codexThemeTitle: 'Тема Codex',
  codexThemeDesc: 'Применяет терминальную палитру Codex (почти чёрный холст, фирменный зелёный акцент) ко всему интерфейсу Harness.',
  codexThemeApply: 'Применить тему Codex',
  codexThemeRestore: 'Следовать светлой/тёмной теме системы',
  codexThemeActive: 'Тема Codex активна',
  codexSessions: 'Сессии',
  codexActiveSessions: 'Текущие сессии',
  codexArchived: 'В архиве',
  codexArchivedEmpty: 'Нет сессий в архиве',
  codexRestore: 'Восстановить',
  codexDelete: 'Удалить',
  codexDeleteConfirm: 'Удалить сессию «{name}» безвозвратно? Журнал переписки этой сессии будет удалён, восстановить его невозможно.',
  codexUnarchiveFailed: 'Не удалось восстановить',
  codexDeleteFailed: 'Не удалось удалить',
  codexTokens: 'Токены дизайна',
  codexTokensDesc: 'Токены интерфейса, которые переопределяет тема Codex: почти чёрный холст, тонкие линии, терминальный зелёный акцент.',
  codexViewerTitle: 'Дизайн Codex',
  codexViewerEmpty: 'В этом файле нет распознаваемых токенов дизайна.',
}
