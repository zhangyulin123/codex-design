/**
 * Client half of codex-design: resolves the user's "Side card"
...
import { registerCodex } from './codex/index.ts'
...
export function apply(ctx: Context): void {
  ...
  }, 'codex-design: better-locale lazy integration')
  ...
  ctx.effect(
    ...
    'codex-design: register built-in tabs and viewers',
  )
  // Codex feature set: the terminal-native Codex theme (with its Settings →
  // General control row), the Codex sidebar page and the design-token viewer.
  // Registered through the same service as the built-ins above, and the theme
  // through a scoped inject on the optional `theme` service, so a host without
  // it still gets the whole workbench.
  ctx.effect(
    () => registerCodex(ctx, service),
    'codex-design: codex theme, page and viewer',
  )
  ...
  const fail = (phase: string, error: unknown): void => {
    console.error(`[codex-design] ${phase} error:`, error)
    try {
      ...
      bar.textContent = `[codex-design] ${phase} error: ${error instanceof Error ? error.message : String(error)}`
  ...
  try {
    ...
    ctx.effect(() => {
      ...
      const scheduleHostCheck = (): void => {
        hostCheckFrame ??= requestAnimationFrame(() => {
          ...
          console.warn('[codex-design] panel host geometry mismatch — a page-level transform was detected; using degraded viewport sync')
      ...
      const mount = (): void => {
        ...
        try {
          ...
          host.setAttribute('data-codex-design', '')
          // Legacy marker, written on purpose (see AGENTS §0 / MIGRATION.md):
          // third-party skins override this plugin's DOM by the pre-merge
          // attribute, so the merged plugin keeps emitting both names.
    ...
    ctx.effect(
      ...
      'codex-design: turn-tail interception',
    )
    ...
    ctx.effect(
      ...
      'codex-design: open-path interception',
    )
    ...
    ctx.effect(
      ...
      'codex-design: link interception',
    )
    ...
    ctx.effect(
      ...
      'codex-design: IME composition guard',
    )
    ...
    ctx.effect(
      ...
      'codex-design: settings navigation icon',
    )
    ...
    // The registration id is a NEW contract (no third-party plugin or
    // persisted user state references it), so it carries the merged package
    // name — unlike the retained runtime constants listed in AGENTS §0.
