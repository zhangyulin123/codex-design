...
export const zhHK: Record<string, string> = {
  ...
  pluginGithubWorkbenchDesc: 'codex-design「GitHub 工作台」Tab：遠端儲存庫目錄樹 + Issues / Pull requests / Actions 頁簽，讀取之外支援新建 Issue/PR、評論、編輯、關閉重開、squash·merge·rebase 合併（強確認）與重跑/取消 CI；儲存庫彈出層自動拉取有權限列表並支援公開儲存庫搜尋；未安裝 codex-design 時自動降級為獨立右側面板',
  ...
  pluginServerDeckDesc: '伺服器卡片儀表板：每台伺服器一張卡片，展示線上狀態、OS、運行時長、CPU/記憶體/磁碟用量與延遲；點卡片進入 xterm.js 互動終端機，支援 ~/.ssh/config 一鍵匯入（自動跳過 Git 託管別名）；安裝 codex-design 後註冊原生「伺服器」分頁，未安裝時保留獨立抽屜',
