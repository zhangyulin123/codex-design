...
export const th: Record<string, string> = {
  ...
  pluginOfficeDesc: 'พรีวิวชุด Office (.docx / .xlsx / .pptx) สำหรับตัวแก้ไข codex-design โดยแยกไลบรารีเรนเดอร์ Office ที่หนักออกจาก bundle หลัก',
  pluginFlowglassDesc: 'กราฟโฟลว์เซสชันสดพร้อมเลนสามเลนสำหรับผู้ใช้, ผู้ช่วย และการเรียกเครื่องมือ พร้อมกลุ่มขนาน, สาขาตัวแทนย่อย, drill-down และสถานะสด; ลงทะเบียน tab Flowglass แบบ native เมื่อติดตั้ง codex-design และคงลิ้นชักสแตนด์อโลนเป็นทางเลือกสำรอง',
  ...
  pluginGithubWorkbenchDesc: 'แท็บ GitHub Workbench: โครงสร้าง repo รีโมต + Issues / Pull requests / Actions พร้อมรองรับการเขียนครบวงจร — สร้าง Issue/PR, แสดงความคิดเห็น, แก้ไข, ปิด/เปิดใหม่, รวมแบบ squash·merge·rebase (ยืนยันก่อนเสมอ), รัน/ยกเลิก CI ใหม่; ตัวสลับ repo จะดึงรายการ repo ที่มีสิทธิ์เข้าถึงโดยอัตโนมัติและค้นหา repo สาธารณะได้; หากไม่ได้ติดตั้ง codex-design จะสลับไปใช้แผงด้านขวาแบบสแตนด์อโลนอัตโนมัติ',
  pluginMdExportDesc: 'เพิ่มปุ่ม "ส่งออก" ในแถบเครื่องมือ Markdown ของ codex-design: เรนเดอร์ .md ปัจจุบันเป็น HTML แบบสแตนด์อโลนในคลิกเดียว (ตาราง/บล็อกโค้ด/ไดอะแกรม Mermaid แบบอินไลน์ เลย์เอาต์ตามธีมพรีวิว) แล้วบันทึกไว้โฟลเดอร์เดียวกัน หรือส่งออกเป็น PDF ผ่านกล่องโต้ตอบการพิมพ์',
  pluginCodeNavDesc: 'การนำทางพรีวิวโค้ด: ระบุภาษาอัตโนมัติจากประเภทไฟล์และไฮไลต์ไวยากรณ์ โครงร่างสัญลักษณ์ (กรองคลาส/เมธอด/ตัวแปร + กระโดดคลิกเดียว) และค้นหาในไฟล์ (ไฮไลต์ทุกจุดที่ตรง ก่อนหน้า/ถัดไป แยกตัวพิมพ์ใหญ่เล็ก) — รับช่วงพรีวิวไฟล์โค้ดในตัวแก้ไข codex-design',
  pluginSuhuangScrollDesc: 'เชื่อมต่อ Suhuang Scroll Runtime ในเครื่องกับการตั้งค่า DSH และ codex-design สำหรับการกำหนดค่าโมเดล การทดสอบการเชื่อมต่อ และการควบคุมการตรวจงานต่อเนื่อง โดยต้องติดตั้ง Suhuang Scroll Runtime และ codex-design',
  pluginBetterOverleafDesc: 'แท็บ Overleaf สำหรับ codex-design: เข้าสู่ระบบผ่านเบราว์เซอร์ CDP โดยตรง (รองรับ Chromium ของบุคคลที่สาม) รายการ/สลับโปรเจกต์ มิเรอร์ git ภายในเครื่องที่ <workspace>/overleaf/ ซิงก์ git สองทางพร้อมสำรองแบบ API อ่านอย่างเดียว และดูตัวอย่างไฟล์ผ่านเวิร์กเบนช์ของแถบข้าง',
  ...
  pluginVideoPreviewDesc: 'พรีวิววิดีโอแบบอินไลน์ (.mp4/.webm/.mov/.mkv/.avi ฯลฯ) สำหรับตัวแก้ไข codex-design โดยสนับสนุนด้วย route /video โฮสต์เฉพาะพร้อมรองรับ HTTP Range (206) — การ scrub ทำงานได้และไฟล์ไม่จำกัดโดย mediaLimit 20MB',
  ...
  pluginEgoBrowserDesc: 'เบราว์เซอร์เอเจนต์สำหรับ DeepSeek Harness: เครื่องมือ ego_* 32 ตัวขับเคลื่อน Chromium จริง แท็บ «ego browser» เนทีฟในแถบด้านข้างแสดงทุกหน้าที่เอเจนต์เยี่ยมชมแบบสด — คลิก ลาก และพิมพ์เพื่อเข้าไปควบคุมแทนได้ ลงทะเบียนแท็บอัตโนมัติเมื่อมี codex-design มิฉะนั้นจะใช้บับเบิลลอย',
  ...
  codexThemeTitle: 'ธีม Codex',
  codexThemeDesc: 'นำชุดสี Codex แบบเทอร์มินัลดั้งเดิม (พื้นเกือบดำ + สีเขียวจุดเน้นอันเป็นเอกลักษณ์) มาใช้กับอินเทอร์เฟซ Harness ทั้งหมด',
  codexThemeApply: 'ใช้ธีม Codex',
  codexThemeRestore: 'ตามโหมดสว่าง/มืดของระบบ',
  codexThemeActive: 'เปิดใช้ธีม Codex อยู่',
  codexSessions: 'เซสชัน',
  codexActiveSessions: 'เซสชันปัจจุบัน',
  codexArchived: 'เก็บถาวรแล้ว',
  codexArchivedEmpty: 'ยังไม่มีเซสชันที่เก็บถาวร',
  codexRestore: 'กู้คืน',
  codexDelete: 'ลบ',
  codexDeleteConfirm: 'ลบเซสชัน «{name}» อย่างถาวรหรือไม่? บันทึกบทสนทนาของเซสชันนี้จะถูกลบและกู้คืนไม่ได้',
  codexUnarchiveFailed: 'กู้คืนไม่สำเร็จ',
  codexDeleteFailed: 'ลบไม่สำเร็จ',
  codexTokens: 'โทเคนดีไซน์',
  codexTokensDesc: 'โทเคนอินเทอร์เฟซที่ธีม Codex เขียนทับ: พื้นเกือบดำ เส้นบาง และสีเขียวแบบเทอร์มินัล',
  codexViewerTitle: 'ดีไซน์ Codex',
  codexViewerEmpty: 'ไม่พบโทเคนดีไซน์ที่รู้จักในไฟล์นี้',
}
