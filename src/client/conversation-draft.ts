...
export function appendToDraft(ctx: Context, sessionId: string, text: string): boolean {
  try {
    ...
    if (actx === undefined) {
      console.warn('[codex-design] draft insert skipped: no session scope', sessionId)
      return false
    }
    const conversation = ctx.get('conversation') as SidebarConversation | undefined
    if (conversation === undefined) {
      console.warn('[codex-design] draft insert skipped: conversation service unavailable')
      return false
    }
    ...
    console.warn('[codex-design] draft insert failed:', error)
    return false
  }
}
...
export function insertFileReference(ctx: Context, sessionId: string, relativePath: string): boolean {
  ...
  try {
    ...
    console.warn('[codex-design] file-reference insert failed:', error)
    return false
  }
}
