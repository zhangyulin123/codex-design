...
export const ja: Record<string, string> = {
  ...
  pluginMdExportDesc: 'codex-design の Markdown ツールバーに「エクスポート」ボタンを追加：現在の .md を単体 HTML（テーブル・コードブロック・Mermaid 図表をインライン化、プレビューテーマに追従したレイアウト）にレンダリングして .md と同じディレクトリに保存、または印刷ダイアログ経由で PDF にエクスポート',
  pluginOfficeDesc: 'codex-design エディター向け Office スイートプレビュー（.docx / .xlsx / .pptx）。重い Office レンダリングライブラリをコアバンドルから分離、必要に応じてインストール',
  pluginFlowglassDesc: 'ライブセッションフローグラフ：ユーザー、アシスタント、ツール呼び出しの 3 レーン泳道、並列グループ、サブエージェント支線、ドリルダウン、ライブ状態をサポート。codex-design インストール時にネイティブ「Flowglass」Tab を登録、未インストール時は独立ドロワーを保持',
  pluginGitForgeDesc: 'codex-design「Git Forge」Tab：GitHub/Gitea 等 Forge アカウントライブラリ + プロジェクト単位の認可 + push ポリシー硬核拒絶。token はローカル secrets のみ、モデルコンテキストに入らない。読み取り専用 GitForge ツールと agent HTTPS credential helper を提供',
  pluginGithubWorkbenchDesc: 'codex-design「GitHub ワークベンチ」タブ:リモートリポジトリのツリー + Issues / Pull requests / Actions タブ。読み取りに加え、Issue・PR の作成、コメント、編集、クローズ/再オープン、squash・merge・rebase マージ(強確認)、CI の再実行/キャンセルに対応。リポジトリ切替ポップアップはアクセス可能なリポジトリを自動取得し、公開リポジトリの検索にも対応。codex-design 未導入時は独立右パネルへ自動フォールバック',
  pluginSuhuangScrollDesc: 'ローカルの Suhuang Scroll Runtime を DSH 設定と codex-design に接続し、モデル設定、接続テスト、連続採点の操作を提供。Suhuang Scroll Runtime と codex-design が必要',
  pluginBetterOverleafDesc: 'codex-design の Overleaf タブ：CDP ブラウザーに直接接続してログイン（サードパーティー Chromium 対応）、プロジェクト一覧/切替、<workspace>/overleaf/ のローカル git ミラー、双方向 git 同期（API 読み取り専用フォールバック）、ファイルプレビューはサイドバーのワークベンチを利用',
  pluginGitRemotesDesc: 'codex-design Git リモート Tab：ブランチ/アップストリーム/ahead-behind 表示、fetch（prune 可）、ff-only pull、確認後のみ push。内蔵 Git のステージ/コミットは置き換えず、force-push やモデル自動 push ツールは提供しない',
  ...
  pluginServerDeckDesc: 'サーバーカードダッシュボード：各サーバーを1枚のカードで表示し、オンライン状態・OS・稼働時間・CPU/メモリ/ディスク使用率とレイテンシを確認。カードをクリックすると xterm.js の対話ターミナルを開き、~/.ssh/config をワンクリック取り込み（Git ホスティングの別名は自動スキップ）。codex-design インストール時はネイティブな「サーバー」タブを登録、未インストール時は独立ドロワーを維持',
  pluginSidebarQaDesc: 'codex-design ベースの選択して質問 Tab：会話選択 → 右パネルで質問 → 同ワークスペースの独立追問会話（❓追問・主題）。高速 no-thinking モデルが主対話コンテキストを圧縮し引用と一緒に注入、主対話を中断しない。追問はネスト、継続、アーカイブ可能',
  ...
  pluginSshTunnelDesc: 'codex-design「SSH トンネル」Tab：複数ホストインベントリ + プロジェクト単位の認可 + 鍵のローカル保管。モデルツール SSHManager（exec/SFTP/セッション戦略）。中央対話ターミナルと双欄 SFTP',
  ...
  pluginVideoPreviewDesc: 'codex-design エディター内で動画ファイルをインラインプレビュー（.mp4/.webm/.mov/.mkv/.avi 等）。HTTP Range (206) をサポートする専用 /video ホストルートを搭載、プログレスバードラッグ可、20MB mediaLimit 制限なし',
  pluginCodeNavDesc: 'コードプレビューナビゲーター：ファイルタイプから言語を自動判定してシンタックスハイライト、シンボルアウトライン（クラス/メソッド/変数のフィルタ＆ワンクリックジャンプ）、ファイル内検索（全一致ハイライト、前/次へ、大文字小文字区別）。codex-design のコードファイルプレビューを引き継ぎます',
  ...
  pluginEgoBrowserDesc: 'DeepSeek Harness 用エージェントブラウザ：32 個の ego_* ツールが本物の Chromium を操作し、サイドバーのネイティブ「ego ブラウザ」Tab がエージェントの訪れる全ページをライブ表示。クリック・ドラッグ・入力で操作を引き継げます。codex-design があれば Tab を自動登録、なければフローティングバブルにフォールバック',
  pluginBilingualReaderDesc: 'DSH サイドバーで論文 PDF を読む：ネイティブ PDF 表示、テキストを選択して LLM で翻訳。コンテキストを使い、メイン会話から完全に分離',
  codexPageTitle: 'Codex',
  codexThemeTitle: 'Codex テーマ',
  codexThemeDesc: 'ターミナル本来の Codex 配色（ほぼ黒のキャンバス＋象徴的なグリーンのアクセント）を Harness の UI 全体に適用します。',
  codexThemeApply: 'Codex テーマを適用',
  codexThemeRestore: 'システムのライト／ダークに追従',
  codexThemeActive: 'Codex テーマを適用中',
  codexSessions: 'セッション',
  codexActiveSessions: '現在のセッション',
  codexArchived: 'アーカイブ済み',
  codexArchivedEmpty: 'アーカイブされたセッションはありません',
  codexRestore: '復元',
  codexDelete: '削除',
  codexDeleteConfirm: 'セッション「{name}」を完全に削除しますか？このセッションの会話ログは削除され、復元できません。',
  codexUnarchiveFailed: '復元に失敗しました',
  codexDeleteFailed: '削除に失敗しました',
  codexTokens: 'デザイントークン',
  codexTokensDesc: 'Codex テーマが上書きする UI トークン：ほぼ黒のキャンバス、ヘアラインの罫線、ターミナルグリーンのアクセント。',
  codexViewerTitle: 'Codex デザイン',
  codexViewerEmpty: 'このファイルには認識できるデザイントークンがありません。',
}
