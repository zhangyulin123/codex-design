...
export function Sidebar(props: { ctx: Context; store: SidebarStore }) {
    ...
    const connect = (): void => {
      ...
      socket.onclose = () => {
        ...
        if (failures >= FAILURE_LIMIT) {
          console.error('[codex-design] agent-terminals connection failed; stopping reconnect loop', sessionId)
          return
        }
  ...
  useEffect(() => {
    ...
    const connect = (): void => {
      ...
      socket.onclose = () => {
        ...
        failures += 1
        if (failures >= FAILURE_LIMIT) {
          console.error('[codex-design] agent-opens connection failed; stopping reconnect loop', sessionId)
          return
        }
        retry = window.setTimeout(connect, 2000)
      }
  ...
  const tabBadgeOf = (tab: SidebarTab): ReactNode => {
    ...
    try {
      ...
      console.error('[codex-design] tab badge error:', error)
      return null
    }
