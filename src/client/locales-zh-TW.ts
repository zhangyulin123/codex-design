...
export const zhTW: Record<string, string> = {
  ...
  openPlugin: '跳轉',
  copyInstall: '複製安裝命令',
  pluginOfficeDesc: '為 codex-design 編輯器提供 Office 三件套預覽（.docx / .xlsx / .pptx），把重型 Office 渲染庫拆出主套件、按需安裝',
  pluginFlowglassDesc: '即時工作階段流程圖：三列泳道展示使用者、助手與工具呼叫，支援並行分組、子代理支線、逐層鑽取和即時狀態；安裝 codex-design 後註冊原生「流鏡」Tab，未安裝時保留獨立抽屜',
  pluginGitForgeDesc: 'codex-design「Git 憑證」Tab：GitHub/Gitea 等 Forge 帳號庫 + 按專案授權 + push 策略硬攔；token 僅存本地 secrets，不進模型上下文；提供唯讀 GitForge 工具與 agent HTTPS credential helper',
  pluginGithubWorkbenchDesc: 'codex-design「GitHub 工作台」Tab：遠端儲存庫目錄樹 + Issues / Pull requests / Actions 頁籤，讀取之外支援建立 Issue/PR、留言、編輯、關閉重開、squash·merge·rebase 合併（強確認）與重跑/取消 CI；儲存庫彈出視窗自動載入有權限清單並支援公開儲存庫搜尋；未安裝 codex-design 時自動降級為獨立右側面板',
  pluginMdExportDesc: '在 codex-design 的 Markdown 工具列上新增「匯出」按鈕：一鍵把目前 .md 渲染為獨立 HTML（表格/程式碼區塊/Mermaid 圖表內嵌，版面跟隨預覽主題）直接儲存到同目錄，或透過列印對話框匯出為 PDF',
  pluginCodeNavDesc: '程式碼預覽導覽：按檔案類型自動識別語言並高亮語法，符號大綱（類別/方法/變數篩選 + 一鍵跳轉），檔案內搜尋（全部符合高亮、上/下一處、區分大小寫），接管 codex-design 的程式碼檔案預覽',
  pluginSuhuangScrollDesc: '把本機蘇黃共閱 Runtime 接入 DSH 設定與 codex-design，支援模型設定、連線測試及連續閱卷控制；使用前須安裝 Suhuang Scroll Runtime 與 codex-design',
  pluginBetterOverleafDesc: 'codex-design 的 Overleaf 分頁：直連 CDP 瀏覽器登入（支援第三方 Chromium），專案列表/切換，<workspace>/overleaf/ 本地 git 映像，git 雙向同步（API 唯讀兜底），檔案預覽走側邊欄工作台',
  pluginGitRemotesDesc: 'codex-design Git 遠端 Tab：看分支/上游/ahead-behind，fetch（可 prune）、ff-only pull、確認後才 push。不替換內建 Git 的暫存/提交，也不提供 force-push 或模型自動推送',
  pluginSentinelDesc: '條件驅動的 agent 喚醒系統：檔案/程序/連接埠/HTTP/命令/webhook 感測器，條件達成自動喚醒休眠工作階段；註冊「哨兵」Tab 展示伺服器全域監控表',
  pluginServerDeckDesc: '伺服器卡片儀表板：每台伺服器一張卡片，展示線上狀態、OS、執行時間、CPU/記憶體/磁碟用量與延遲；點擊卡片開啟 xterm.js 互動終端機，支援 ~/.ssh/config 一鍵匯入（自動略過 Git 託管別名）；安裝 codex-design 後註冊原生「伺服器」分頁，未安裝時保留獨立抽屜',
  pluginSidebarQaDesc: '基於 codex-design 的劃選提問tab分頁: 對話劃選 → 右側面板提問 → 同工作區獨立追問工作階段（❓追問·主題）：快速無思考模型壓縮主對話上下文後與引文一起注入，不打斷主對話；追問可巢狀、可繼續、可歸檔',
  ...
  pluginSshTunnelDesc: 'codex-design「SSH 隧道」Tab：多機主機清單 + 按專案授權 + 密鑰本地保管；模型工具 SSHManager（exec/SFTP/工作階段策略）；中央互動終端與雙欄 SFTP',
  ...
  pluginVideoPreviewDesc: '在 codex-design 編輯器內聯預覽影片檔案（.mp4/.webm/.mov/.mkv/.avi 等），自帶支援 HTTP Range（206）的 /video 宿主路由，可拖曳進度條、不受 20MB mediaLimit 限制',
  pluginDocsPanelDesc: 'DSH 側邊欄裡的「全域文件」：全域 Markdown 筆記，任何工作區隨時可讀——列表點選閱讀、懸浮大綱跳轉、Chrome / VS Code 外部開啟、程式碼複製，目錄可設定（預設 ~/.dsh/docs）',
  pluginEgoBrowserDesc: '將 CitroLabs/ego-lite 接進 DeepSeek Harness 的 agent 瀏覽器：32 個 ego_* 工具驅動真實 Chromium，側邊欄原生「ego 瀏覽器」Tab 實時觀察 agent 逛的每個頁面，可直接點擊/拖拽/輸入接管；裝 codex-design 時自動註冊 Tab，沒裝則退回浮動浮窗',
  pluginBilingualReaderDesc: '在 DSH 側邊欄讀論文 PDF：原生 PDF 顯示，選取一段文字即用大模型劃詞翻譯，結合上下文、完全隔離主對話，只作閱讀輔助',
  codexPageTitle: 'Codex',
  codexThemeTitle: 'Codex 主題',
  codexThemeDesc: '將終端原生的 Codex 配色（近黑畫布 + 標誌性綠色點綴）套用到整個 Harness 介面。',
  codexThemeApply: '套用 Codex 主題',
  codexThemeRestore: '跟隨系統亮/暗模式',
  codexThemeActive: 'Codex 主題已生效',
  codexSessions: '工作階段',
  codexActiveSessions: '目前工作階段',
  codexArchived: '已歸檔',
  codexArchivedEmpty: '暫無已歸檔工作階段',
  codexRestore: '復原',
  codexDelete: '刪除',
  codexDeleteConfirm: '確定要永久刪除工作階段「{name}」嗎？該工作階段的對話記錄會被刪除，且無法復原。',
  codexUnarchiveFailed: '復原失敗',
  codexDeleteFailed: '刪除失敗',
  codexTokens: '設計權杖',
  codexTokensDesc: 'Codex 主題覆寫的介面權杖：近黑畫布、細線描邊、終端綠點綴。',
  codexViewerTitle: 'Codex 設計稿',
  codexViewerEmpty: '這個檔案裡沒有可辨識的設計權杖。',
}
