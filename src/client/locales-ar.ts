...
export const ar: Record<string, string> = {
  ...
  pluginOfficeDesc: 'معاينة حزمة Office (.docx / .xlsx / .pptx) لمحرّر codex-design، مع إبقاء مكتبات عرض Office الثقيلة خارج الحزمة الأساسية، تُثبَّت عند الطلب',
  pluginFlowglassDesc: 'مخطط تدفق جلسة حيّ بثلاثة مسارات للمستخدم والمساعد واستدعاءات الأدوات، مع مجموعات متوازية وفروع وكلاء فرعيين وتعمّق تدريجي وحالة حيّة؛ يُسجِّل تبويب Flowglass الأصلي عند تثبيت codex-design ويحتفظ بدرجه المستقل كاحتياط',
  ...
  pluginGithubWorkbenchDesc: 'تبويب GitHub Workbench: شجرة المستودع البعيد + Issues / Pull requests / Actions مع دعم كامل للكتابة — إنشاء Issue/PR، والتعليق، والتعديل، والإغلاق/إعادة الفتح، والدمج عبر squash·merge·rebase (بتأكيد قوي)، وإعادة تشغيل CI أو إلغائها؛ يعرض مبدّل المستودعات تلقائيًا المستودعات المتاحة ويتيح البحث في المستودعات العامة؛ وبدون codex-design يتحول تلقائيًا إلى لوحة جانبية مستقلة على اليمين',
  pluginMdExportDesc: 'يضيف زر «تصدير» إلى شريط أدوات Markdown في codex-design: يعرض ملف .md الحالي بنقرة واحدة كملف HTML مستقل (الجداول/كتل الأكواد/مخططات Mermaid مدمجة، والتخطيط يتبع سمة المعاينة) ويحفظه بجوار الملف، أو يصدّره PDF عبر نافذة الطباعة',
  pluginCodeNavDesc: 'تنقل معاينة الكود: يكتشف اللغة تلقائياً من نوع الملف ويبرز الصياغة، مخطط الرموز (تصفية الفئات/الطرق/المتغيرات + قفزة بنقرة واحدة)، والبحث داخل الملف (إبراز كل التطابقات، السابق/التالي، مطابقة حالة الأحرف) — يتولى معاينة ملفات الكود في محرر codex-design',
  pluginSuhuangScrollDesc: 'يربط بيئة Suhuang Scroll المحلية بإعدادات DSH وbetter-sidebar لإعداد النموذج واختبار الاتصال والتحكم في التصحيح المتواصل؛ يتطلب Suhuang Scroll Runtime وcodex-design',
  pluginBetterOverleafDesc: 'تبويب Overleaf لـ codex-design: تسجيل دخول مباشر عبر متصفح CDP (يدعم Chromium من جهات أخرى)، قائمة المشاريع/التبديل، نسخ git محلية ضمن <workspace>/overleaf/، مزامنة git ثنائية الاتجاه مع تراجع إلى API للقراءة فقط، ومعاينة الملفات عبر مساحة عمل الشريط الجانبي',
  ...
  pluginVideoPreviewDesc: 'معاينة فيديو مضمّنة (.mp4/.webm/.mov/.mkv/.avi إلخ) في محرّر codex-design، مدعومة بمسار /video مخصّص يدعم HTTP Range (206) — التحكم بالتشغيل يعمل والملفات غير مقيدة بحد mediaLimit البالغ 20MB',
  ...
  pluginEgoBrowserDesc: 'متصفح الوكيل لـ DeepSeek Harness: 32 أداة ego_* تقود متصفح Chromium حقيقي، مع تبويب «متصفح ego» أصلي في الشريط الجانبي يعرض مباشرة كل صفحة يزورها الوكيل — يمكنك النقر والسحب والكتابة لتولي التحكم. يسجّل التبويب تلقائيًا عند وجود codex-design، وإلا يظهر كفقاعة عائمة',
  ...
  codexThemeTitle: 'سمة Codex',
  codexThemeDesc: 'يطبّق لوحة ألوان Codex الأصلية في الطرفية (خلفية شبه سوداء ولمسة خضراء مميزة) على واجهة Harness بالكامل.',
  codexThemeApply: 'تطبيق سمة Codex',
  codexThemeRestore: 'اتّباع الوضع الفاتح/الداكن للنظام',
  codexThemeActive: 'سمة Codex مفعّلة',
  codexSessions: 'الجلسات',
  codexActiveSessions: 'الجلسات الحالية',
  codexArchived: 'المؤرشفة',
  codexArchivedEmpty: 'لا توجد جلسات مؤرشفة',
  codexRestore: 'استعادة',
  codexDelete: 'حذف',
  codexDeleteConfirm: 'هل تريد حذف الجلسة «{name}» نهائيًا؟ سيُحذف سجل المحادثة الخاص بهذه الجلسة ولا يمكن استعادته.',
  codexUnarchiveFailed: 'فشلت الاستعادة',
  codexDeleteFailed: 'فشل الحذف',
  codexTokens: 'رموز التصميم',
  codexTokensDesc: 'رموز الواجهة التي تستبدلها سمة Codex: خلفية شبه سوداء، وحدود رفيعة، ولمسة خضراء بلون الطرفية.',
  codexViewerTitle: 'تصميم Codex',
  codexViewerEmpty: 'لا توجد رموز تصميم يمكن التعرّف عليها في هذا الملف.',
}
