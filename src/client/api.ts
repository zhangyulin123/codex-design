...
export const api = {
  ...
  openExternal,
  /** Durable Codex-theme intent (a host file, not localStorage: DSH Desktop
   *  serves the client from a random per-launch loopback port, so browser
   *  localStorage is empty on every launch). `active: null` = no state file
   *  yet (fresh install), distinct from a known `false`. */
  codexStateGet: () =>
    call<{ available: boolean; active: boolean | null }>('codex.state.get', {}),
  /** Persist the Codex-theme intent. */
  codexStateSet: (active: boolean) =>
    call<{ active: boolean }>('codex.state.set', { active }),
  /** The durable archive set (host-authoritative; rc.1 exposes no client
   *  workspace service to read it from). */
  codexSessionsArchived: () =>
    call<{ available: boolean; ids: string[] }>('codex.sessions.archived', {}),
  /** Restore one archived session (the core UI has no unarchive surface). */
  codexSessionsUnarchive: (sessionId: string) =>
    call<{ unarchived: boolean }>('codex.sessions.unarchive', { sessionId }),
  /** Fallback session teardown, used only when the native
   *  `ctx.sessions.delete()` remote is unwired in this build. */
  codexSessionsDelete: (sessionId: string) =>
    call<{ deleted: boolean }>('codex.sessions.delete', { sessionId }),
}
