...
export const vi: Record<string, string> = {
  ...
  pluginOfficeDesc: 'Xem trước bộ Office (.docx / .xlsx / .pptx) cho trình soạn thảo codex-design, tách thư viện render Office nặng ra khỏi bundle chính, cài theo nhu cầu',
  pluginFlowglassDesc: 'Biểu đồ luồng phiên theo thời gian thực: ba lane cho user, assistant và tool call, hỗ trợ nhóm song song, nhánh tác nhân con, drill-down từng lớp và trạng thái trực tiếp; sau khi cài codex-design đăng ký tab «Flowglass» native, khi chưa cài giữ drawer riêng',
  pluginGitForgeDesc: 'Tab «Git Forge» của codex-design: kho tài khoản GitHub/Gitea và Forge khác + cấp quyền theo project + chặn cứng chính sách push; token chỉ lưu secrets cục bộ, không vào ngữ cảnh mô hình; cung cấp công cụ GitForge chỉ đọc và HTTPS credential helper cho agent',
  pluginGithubWorkbenchDesc: 'Tab GitHub Workbench: cây repo từ xa + Issues / Pull requests / Actions với hỗ trợ ghi đầy đủ — tạo Issue/PR, bình luận, chỉnh sửa, đóng/mở lại, merge squash·merge·rebase (xác nhận mạnh), chạy lại/hủy CI; bộ chuyển repo tự liệt kê các repo có quyền truy cập và tìm kiếm repo công khai; khi chưa cài codex-design sẽ tự động chuyển sang panel bên phải độc lập',
  pluginMdExportDesc: 'Thêm nút «Xuất» vào thanh công cụ Markdown của codex-design: một cú nhấp chuyển .md hiện tại thành HTML độc lập (bảng/khối mã/sơ đồ Mermaid nhúng, bố cục theo theme xem trước) và lưu cạnh tệp .md, hoặc xuất PDF qua hộp thoại in',
  pluginCodeNavDesc: 'Điều hướng xem trước mã: tự nhận diện ngôn ngữ theo loại tệp và tô sáng cú pháp, phác thảo ký hiệu (lọc lớp/phương thức/biến + nhảy một cú nhấp), tìm kiếm trong tệp (tô sáng mọi kết quả, trước/sau, phân biệt hoa thường) — đảm nhận xem trước tệp mã trong trình soạn thảo codex-design',
  pluginSuhuangScrollDesc: 'Kết nối Suhuang Scroll Runtime cục bộ với phần cài đặt DSH và codex-design để cấu hình mô hình, kiểm tra kết nối và điều khiển chấm bài liên tục; yêu cầu Suhuang Scroll Runtime và codex-design',
  pluginBetterOverleafDesc: 'Tab Overleaf cho codex-design: đăng nhập trực tiếp qua trình duyệt CDP (hỗ trợ Chromium bên thứ ba), danh sách/chuyển dự án, mirror git cục bộ trong <workspace>/overleaf/, đồng bộ git hai chiều với dự phòng API chỉ đọc, xem trước tệp qua workbench của thanh bên',
  pluginGitRemotesDesc: 'Tab Git Remotes của codex-design: xem branch/upstream/ahead-behind, fetch (có thể prune), pull ff-only, push chỉ sau khi xác nhận trong tab. Không thay thế tab stage/commit Git built-in, cũng không cung cấp force-push hay công cụ auto-push cho mô hình',
  ...
  pluginServerDeckDesc: 'Bảng điều khiển thẻ máy chủ: một thẻ cho mỗi host hiển thị trạng thái trực tuyến, hệ điều hành, thời gian hoạt động, mức dùng CPU/bộ nhớ/ổ đĩa và độ trễ; nhấp vào thẻ để mở terminal xterm.js tương tác; nhập ~/.ssh.config một chạm (tự bỏ qua bí danh dịch vụ lưu trữ Git). Đăng ký tab 「Máy chủ」 gốc khi codex-design được cài, ngược lại giữ ngăn kéo độc lập',
  pluginSidebarQaDesc: 'Tab chọn-đoạn-hỏi dựa trên codex-design: chọn văn bản trò chuyện → hỏi ở panel phải → phiên theo dõi riêng trong cùng workspace (❓ theo dõi · chủ đề): mô hình nhanh không suy nghĩ nén ngữ cảnh chat chính rồi tiêm cùng trích dẫn, không gián đoạn chat chính; theo dõi có thể lồng nhau, tiếp tục, lưu trữ',
  ...
  pluginSshTunnelDesc: 'Tab «SSH Tunnel» của codex-design: danh sách máy đa host + cấp quyền theo project + khóa lưu cục bộ; công cụ mô hình SSHManager (exec/SFTP/chiến lược phiên); terminal tương tác trung tâm và SFTP hai pane',
  ...
  pluginVideoPreviewDesc: 'Xem trước video inline trong trình soạn thảo codex-design (.mp4/.webm/.mov/.mkv/.avi v.v.), có route host /video hỗ trợ HTTP Range (206), có thể tua tiến độ, không bị giới hạn mediaLimit 20MB',
  ...
  pluginEgoBrowserDesc: 'Trình duyệt agent cho DeepSeek Harness: 32 công cụ ego_* điều khiển Chromium thật; tab «ego browser» gốc trong thanh bên hiển thị trực tiếp mọi trang agent truy cập — bạn có thể bấm, kéo và gõ để tiếp quản. Tự động đăng ký tab khi có codex-design, nếu không thì rơi về bong bóng nổi',
  ...
  codexThemeTitle: 'Chủ đề Codex',
  codexThemeDesc: 'Áp dụng bảng màu Codex gốc của terminal (nền gần đen, điểm nhấn xanh đặc trưng) cho toàn bộ giao diện Harness.',
  codexThemeApply: 'Áp dụng chủ đề Codex',
  codexThemeRestore: 'Theo sáng / tối của hệ thống',
  codexThemeActive: 'Chủ đề Codex đang bật',
  codexSessions: 'Phiên',
  codexActiveSessions: 'Phiên hiện tại',
  codexArchived: 'Đã lưu trữ',
  codexArchivedEmpty: 'Chưa có phiên nào được lưu trữ',
  codexRestore: 'Khôi phục',
  codexDelete: 'Xóa',
  codexDeleteConfirm: 'Xóa vĩnh viễn phiên «{name}»? Nhật ký hội thoại của phiên này sẽ bị xóa và không thể khôi phục.',
  codexUnarchiveFailed: 'Khôi phục thất bại',
  codexDeleteFailed: 'Xóa thất bại',
  codexTokens: 'Token thiết kế',
  codexTokensDesc: 'Các token giao diện mà chủ đề Codex ghi đè: nền gần đen, nét mảnh, điểm nhấn xanh terminal.',
  codexViewerTitle: 'Thiết kế Codex',
  codexViewerEmpty: 'Không có token thiết kế nào nhận diện được trong tệp này.',
}
