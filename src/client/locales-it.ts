 ...
 *
 * @module codex-design/client/locales-it
 */
...
export const it: Record<string, string> = {
  ...
  pluginOfficeDesc: 'Anteprima della suite Office (.docx / .xlsx / .pptx) per l’editor di codex-design, tenendo le pesanti librerie di rendering Office fuori dal bundle principale, installabili su richiesta',
  pluginFlowglassDesc: 'Diagramma di flusso live della sessione con tre corsie per utente, assistente e chiamate agli strumenti, più gruppi paralleli, rami dei sottoagenti, drill-down e stato in tempo reale; registra una scheda nativa Flowglass quando codex-design è installato e mantiene il suo cassetto autonomo come fallback',
  ...
  pluginGithubWorkbenchDesc: 'Scheda GitHub Workbench: albero del repository remoto + Issues / Pull requests / Actions con pieno supporto in scrittura — creazione di issue/PR, commenti, modifica, chiusura/riapertura, merge squash·merge·rebase (conferma rafforzata), riavvio/annullamento della CI; il selettore dei repository elenca automaticamente quelli accessibili e cerca tra quelli pubblici; senza codex-design esegue il fallback a un pannello laterale destro indipendente',
  pluginMdExportDesc: 'Aggiunge un pulsante «Esporta» alla barra degli strumenti Markdown di codex-design: con un clic esegue il rendering del .md corrente in HTML autonomo (tabelle/blocchi di codice/diagrammi Mermaid in linea, layout fedele al tema di anteprima) e lo salva accanto al .md, oppure esporta in PDF tramite la finestra di stampa',
  pluginCodeNavDesc: 'Navigazione dell’anteprima del codice: rileva il linguaggio dal tipo di file ed evidenzia la sintassi, struttura dei simboli (filtri classi/metodi/variabili + salto con un clic) e ricerca nel file (tutte le corrispondenze evidenziate, precedente/successiva, maiuscole/minuscole) — gestisce l’anteprima dei file di codice nell’editor codex-design',
  pluginSuhuangScrollDesc: 'Collega il Runtime Suhuang Scroll locale alle impostazioni DSH e a codex-design per configurare il modello, testare la connessione e controllare la correzione continua; richiede Suhuang Scroll Runtime e codex-design',
  pluginBetterOverleafDesc: 'Tab Overleaf per codex-design: accesso diretto tramite browser CDP (supporta Chromium di terze parti), elenco/cambio progetti, mirror git locali in <workspace>/overleaf/, sincronizzazione git bidirezionale con fallback API di sola lettura e anteprima dei file tramite la workbench della barra laterale',
  ...
  pluginVideoPreviewDesc: 'Anteprima video in linea nell’editor di codex-design (.mp4/.webm/.mov/.mkv/.avi ecc.), supportata da una route host /video dedicata con supporto HTTP Range (206) — la barra di scorrimento funziona e i file non sono limitati dal mediaLimit di 20MB',
  ...
  pluginEgoBrowserDesc: 'Il browser agente per DeepSeek Harness: 32 strumenti ego_* pilotano un vero Chromium; una scheda nativa «ego browser» nella barra laterale mostra in tempo reale ogni pagina visitata dall’agente — puoi cliccare, trascinare e digitare per prendere il controllo. Registra la scheda automaticamente se codex-design è presente, altrimenti una bolla flottante',
  ...
  codexThemeTitle: 'Tema Codex',
  codexThemeDesc: 'Applica la palette Codex nativa del terminale (tela quasi nera, accento verde inconfondibile) all’intera interfaccia di Harness.',
  codexThemeApply: 'Applica il tema Codex',
  codexThemeRestore: 'Segui chiaro / scuro di sistema',
  codexThemeActive: 'Il tema Codex è attivo',
  codexSessions: 'Sessioni',
  codexActiveSessions: 'Sessioni correnti',
  codexArchived: 'Archiviate',
  codexArchivedEmpty: 'Nessuna sessione archiviata',
  codexRestore: 'Ripristina',
  codexDelete: 'Elimina',
  codexDeleteConfirm: 'Eliminare definitivamente la sessione «{name}»? Il registro della conversazione di questa sessione verrà rimosso e non potrà essere recuperato.',
  codexUnarchiveFailed: 'Ripristino non riuscito',
  codexDeleteFailed: 'Eliminazione non riuscita',
  codexTokens: 'Token di design',
  codexTokensDesc: 'Token dell’interfaccia che il tema Codex sostituisce: tela quasi nera, tratti sottili, accento verde da terminale.',
  codexViewerTitle: 'Design Codex',
  codexViewerEmpty: 'Nessun token di design riconoscibile in questo file.',
}
