...
export const builtinTabPlugins: readonly PluginEntry[] = [
  ...
  },
  {
    ...
    // codex-design (auto-tab when present, floating bubble when not).
    install: 'cd ~/.dsh && dsh plugin --profile web add codex-design && dsh plugin --profile web add git+https://github.com/Fisfzy/ego-browser.git',
  },
  {
    ...
    // Published on npm; peer-depends on codex-design (Overleaf tab),
    // so the install line installs the prerequisite first.
    install: 'cd ~/.dsh && dsh plugin --profile web add codex-design && dsh plugin --profile web add dsh-better-overleaf',
  },
  {
    id: 'dsh-docs-panel',
    ...
    // dsh-docs-panel hard-depends on codex-design (required peer), so
    // the install line installs the prerequisite first, then the plugin.
    install: 'cd ~/.dsh && dsh plugin --profile web add codex-design && dsh plugin --profile web add dsh-docs-panel',
  },
  {
    id: 'dsh-flowglass',
    ...
    // native tab automatically when codex-design is present.
    install: 'cd ~/.dsh && dsh plugin --profile web add codex-design && dsh plugin --profile web add dsh-flowglass',
  },
  {
    id: 'dsh-git-forge',
    ...
    // Peer-depends on codex-design (Git Forge tab). Install the
    // prerequisite first; package is GitHub-sourced until npm publish.
    install: 'cd ~/.dsh && dsh plugin --profile web add codex-design && dsh plugin --profile web add "dsh-git-forge@github:thirsty5034/dsh-git-forge"',
  },
  {
    id: 'dsh-git-remotes',
    ...
    install: 'cd ~/.dsh && dsh plugin --profile web add codex-design && dsh plugin --profile web add git+https://github.com/yq04/dsh-git-remotes.git',
  },
  {
  ...
  },
  {
    ...
    // dsh-sidebar-qa hard-depends on codex-design (required peer), so
    // the install line installs the prerequisite first, then the plugin.
    install: 'cd ~/.dsh && dsh plugin --profile web add codex-design && dsh plugin --profile web add git+https://github.com/ChenRuoT/dsh-sidebar-qa.git',
  },
  {
    id: 'dsh-sidenote',
    ...
    // dsh-sidenote hard-depends on codex-design (required peer), so
    // the install line installs the prerequisite first, then the plugin.
    install: 'cd ~/.dsh && dsh plugin --profile web add codex-design && dsh plugin --profile web add dsh-sidenote',
  },
  {
    id: 'dsh-server-deck',
    ...
    // "Servers" tab when codex-design is present, standalone drawer
    // otherwise. Install the prerequisite first.
    install: 'cd ~/.dsh && dsh plugin --profile web add codex-design && dsh plugin --profile web add dsh-server-deck@latest',
  },
  {
    ...
    // codex-design. Install the sidebar prerequisite before the npm package.
    install: 'cd ~/.dsh && dsh plugin --profile web add codex-design && dsh plugin --profile web add dsh-suhuang-scroll',
  },
  {
    id: 'dsh-ssh-tunnel',
    ...
    // Peer-depends on codex-design (SSH Tunnel tab + center terminal/SFTP).
    // Install the prerequisite first; package is GitHub-sourced until npm publish.
    install: 'cd ~/.dsh && dsh plugin --profile web add codex-design && dsh plugin --profile web add "dsh-ssh-tunnel@github:thirsty5034/dsh-ssh-tunnel"',
  },
  {
    id: 'dsh-turn-review',
    ...
    // Needs codex-design (optional peer) for the tab; no model tools.
    install: 'cd ~/.dsh && dsh plugin --profile web add codex-design && dsh plugin --profile web add git+https://github.com/yq04/dsh-turn-review.git',
  },
  {
    ...
    // codex-design tab service, so install the prerequisite first.
    install: 'cd ~/.dsh && dsh plugin --profile web add codex-design && dsh plugin --profile web add github:Johnblur/dsh-bilingual-reader',
  },
]
