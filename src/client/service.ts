...
export function matchUrlTarget(tabs: readonly TabDescriptor[], url: URL): TabDescriptor | undefined {
  for (const tab of tabs) {
    ...
    try {
      ...
      console.error('[codex-design] urlTarget error:', error)
      continue
    }
 ...
 *
 * The merged package (codex-design v0.19.0) inherits the sidebar service API
 * unchanged from dsh-better-sidebar v0.18.0: this string is the *service*
 * version, not a re-introduction of the old package name.
 */
...
function safeCall(fn: () => void): void {
  try {
    ...
    console.error('[codex-design] plugin callback error:', error)
  }
}
...
export function createBetterSidebarService(store: SidebarStore): BetterSidebarService {
  ...
  const registerTab = (descriptor: TabDescriptor): (() => void) => {
    if (tabs.has(descriptor.id)) {
      throw new Error(`[codex-design] tab type "${descriptor.id}" already registered`)
    }
  ...
  const registerFileViewer = (descriptor: FileViewerDescriptor): (() => void) => {
    if (viewers.has(descriptor.id)) {
      throw new Error(`[codex-design] file viewer "${descriptor.id}" already registered`)
    }
  ...
  const openTab = (seed: OpenTabSeed, scope?: SessionScope): void => {
    ...
    if (!isTabEnabled(seed.type)) {
      console.warn(`[codex-design] tab type "${seed.type}" is disabled in the side card settings`)
      return
    }
