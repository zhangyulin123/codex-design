/**
 * Korean dictionary for the codex-design plugin (LOCALE_NS `betterSidebar`).
 *
...
export const ko: Record<string, string> = {
  ...
  pluginOfficeDesc: 'codex-design 편집기에 Office 3종 미리보기(.docx / .xlsx / .pptx)를 제공하며, 무거운 Office 렌더링 라이브러리를 메인 패키지에서 분리하여 필요할 때 설치합니다',
  pluginFlowglassDesc: '실시간 세션 흐름도: 사용자, 어시스턴트, 도구 호출을 3열 레인으로 표시하며, 병렬 그룹, 서브 에이전트 지선, 단계별 드릴다운과 실시간 상태를 지원합니다. codex-design 설치 시 기본 "흐름 거울(流镜)" Tab을 등록하고, 미설치 시 독립 서랍을 유지합니다',
  pluginGitForgeDesc: 'codex-design "Git 자격 증명" Tab: GitHub/Gitea 등 Forge 계정 라이브러리 + 프로젝트별 권한 + push 정책 하드 차단. 토큰은 로컬 secrets에만 저장되고 모델 컨텍스트에 들어가지 않습니다. 읽기 전용 GitForge 도구와 agent HTTPS credential helper를 제공합니다',
  pluginGithubWorkbenchDesc: 'codex-design "GitHub Workbench" Tab: 원격 저장소 트리 + Issues / Pull requests / Actions 탭. 읽기 외에 Issue·PR 생성, 댓글, 편집, 닫기/다시 열기, squash·merge·rebase 병합(강한 확인), CI 재실행/취소를 지원합니다. 저장소 전환 팝업은 접근 가능한 저장소를 자동으로 가져오고 공개 저장소 검색도 지원합니다. codex-design 미설치 시 독립 우측 패널로 자동 폴백합니다',
  ...
  pluginCodeNavDesc: '코드 미리보기 내비게이션: 파일 형식으로 언어를 자동 인식해 구문 강조, 심볼 아웃라인(클래스/메서드/변수 필터 + 원클릭 이동), 파일 내 검색(모든 일치 강조, 이전/다음, 대소문자 구분) — codex-design 편집기의 코드 파일 미리보기를 대신 맡습니다',
  pluginSuhuangScrollDesc: '로컬 Suhuang Scroll Runtime을 DSH 설정과 better-sidebar에 연결하여 모델 구성, 연결 테스트, 연속 채점 제어를 제공합니다. Suhuang Scroll Runtime과 codex-design가 필요합니다',
  ...
  pluginGitRemotesDesc: 'codex-design Git 원격 Tab: 브랜치/업스트림/ahead-behind 확인, fetch(prune 가능), ff-only pull, 확인 후에만 push. 내장 Git의 스테이징/커밋을 대체하지 않으며, force-push나 모델 자동 push도 제공하지 않습니다',
  ...
  pluginServerDeckDesc: '서버 카드 대시보드: 호스트마다 한 장의 카드로 온라인 상태, OS, 업타임, CPU/메모리/디스크 사용률과 지연 시간을 표시합니다. 카드를 클릭하면 xterm.js 대화형 터미널이 열리고, ~/.ssh/config를 원클릭으로 가져옵니다(Git 호스팅 별칭은 자동 제외). codex-design 설치 시 네이티브 「서버」 탭을 등록하고, 미설치 시 독립 드로어를 유지합니다',
  pluginSidebarQaDesc: 'codex-design 기반 드래그 선택 질문 탭: 대화 드래그 선택 → 오른쪽 패널 질문 → 같은 작업 공간의 독립 후속 질문 세션(❓후속 질문·주제). 빠른 무사고 모델이 주 대화 컨텍스트를 압축한 후 인용과 함께 주입하며, 주 대화를 방해하지 않습니다. 후속 질문은 중첩, 계속, 보관이 가능합니다',
  ...
  pluginSshTunnelDesc: 'codex-design "SSH 터널" Tab: 다중 호스트 목록 + 프로젝트별 권한 + 키 로컬 보관. 모델 도구 SSHManager(exec/SFTP/세션 전략). 중앙 대화형 터미널과 이중 창 SFTP',
  ...
  pluginVideoPreviewDesc: 'codex-design 편집기에서 비디오 파일(.mp4/.webm/.mov/.mkv/.avi 등)을 인라인 미리보기합니다. HTTP Range(206)를 지원하는 /video 호스트 라우트를 내장하며, 진행 바를 드래그할 수 있고 20MB mediaLimit 제한을 받지 않습니다',
  ...
  codexThemeTitle: 'Codex 테마',
  codexThemeDesc: '터미널 고유의 Codex 색상(거의 검은 캔버스, 상징적인 녹색 액센트)을 Harness UI 전체에 적용합니다.',
  codexThemeApply: 'Codex 테마 적용',
  codexThemeRestore: '시스템 밝게/어둡게 따르기',
  codexThemeActive: 'Codex 테마가 적용되었습니다',
  codexSessions: '세션',
  codexActiveSessions: '현재 세션',
  codexArchived: '보관됨',
  codexArchivedEmpty: '보관된 세션이 없습니다',
  codexRestore: '복원',
  codexDelete: '삭제',
  codexDeleteConfirm: '세션 "{name}"을(를) 영구적으로 삭제하시겠습니까? 이 세션의 대화 기록이 삭제되며 복구할 수 없습니다.',
  codexUnarchiveFailed: '복원 실패',
  codexDeleteFailed: '삭제 실패',
  codexTokens: '디자인 토큰',
  codexTokensDesc: 'Codex 테마가 재정의하는 UI 토큰: 거의 검은 캔버스, 헤어라인 테두리, 터미널 녹색 액센트.',
  codexViewerTitle: 'Codex 디자인',
  codexViewerEmpty: '이 파일에서 인식할 수 있는 디자인 토큰이 없습니다.',
}
