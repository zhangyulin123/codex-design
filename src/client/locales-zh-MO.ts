...
export const zhMO: Record<string, string> = {
  ...
  pluginGithubWorkbenchDesc: 'codex-design「GitHub 工作台」Tab：遠端儲存庫目錄樹 + Issues / Pull requests / Actions 頁簽，讀取之外支援新建 Issue/PR、評論、編輯、關閉重開、squash·merge·rebase 合併（強確認）與重跑/取消 CI；儲存庫彈層自動拉取有權限清單並支援公開儲存庫搜尋；未安裝 codex-design 時自動降級為獨立右側面板',
  ...
  pluginServerDeckDesc: '伺服器卡片儀表板：每台伺服器一張卡片，展示在線狀態、OS、運行時長、CPU/內存/磁盤用量與延遲；點卡片進入 xterm.js 交互終端，支持 ~/.ssh/config 一鍵導入（自動跳過 Git 託管別名）；安裝 codex-design 後註冊原生「服務器」Tab，未安裝時保留獨立抽屜',
